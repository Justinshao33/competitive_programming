...
void solve() {
    int n, m; cin >> n >> m;
    Dinic G(n + 4);
    int s = n, t = s + 1, s2 = t + 1, t2 = s2 + 1;
    int ss, tt; cin >> ss >> tt;
    ss--, tt--;
    G.add_edge(s, ss, IINF);
    G.add_edge(tt, t, IINF);
    vector<int> in(n, 0);
    rep (i, 0, m) {
        int a, b, l, u; cin >> a >> b >> l >> u;
        a--, b--;
        G.add_edge(a, b, u - l);
        in[a] -= l;
        in[b] += l;
    ...
    rep (i, 0, n) {
        if (in[i] < 0) {
            G.add_edge(i, t2, -in[i]);
        } else {
            sum += in[i];
            G.add_edge(s2, i, in[i]);
        }
    }
    G.add_edge(t, s, IINF);
    int check = G.flow(s2, t2);
    if (check < sum) {
