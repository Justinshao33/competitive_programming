...
void solve() {
    ...
    auto dfs = [&](auto self, int u, int pa, int s, int M, int m, int d) -> void {
        if (u != s) {
            int cur = (M >> 1) ^ (m << 1) ^ d;
            if (mp.find(cur) != mp.end() && (mp[cur].F != min(u, s) || mp[cur].S != max(u, s))) {
                cout << mp[cur].F + 1 << ' ' << mp[cur].S + 1 << ' ' << u + 1 << ' ' << s + 1 << '\n';
                exit(0);
            }
            mp[cur] = {min(u, s), max(u, s)};
        }
        for (auto [v, w] : adj[u]) {
            if (v == pa) continue;
            self(self, v, u, s, max(M, w), min(m, w), d + 1);
        }
    };
    for (int i = n - 1; i >= 0; --i) dfs(dfs, i, -1, i, 0, IINF, 1);
    cout << -1 << '\n';
}
