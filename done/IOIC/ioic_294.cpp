...
struct RollingHash {
    ...
    int n, c, mod;
    vector <mint> pre, pw;
    RollingHash(string _s, int _c) : s(_s), n(s.size()), c(_c) {
        pre.resize(n + 1, 0);
        pw.resize(n + 1, 1);
        for (int i = 1; i <= n; ++i) {
            pre[i] = pre[i - 1] * c + s[i - 1];
        }
        for (int i = 1; i <= n; ++i) {
            pw[i] = pw[i - 1] * c;
        }
...
void solve() {
    ...
    RollingHash hash(s, 137);
    int mx = *max_element(all(len));
    vector<int> cand;
    rep (i, 0, n) if (len[i] == mx) cand.pb(i);
    int opt = cand[0];
    rep (i, 1, cand.size()) {
        int l = 0, r = mx;
        while (r - l > 1) {
            int mid = l + r >> 1;
            if (hash.query(opt, opt + mid).val() == hash.query(cand[i], cand[i] + mid).val()) l = mid;
            else r = mid;
        }
        if (c[s[cand[i] + l]] < c[s[opt + l]]) opt = cand[i];
    }
    cout << s.substr(opt, mx) << '\n';
}
