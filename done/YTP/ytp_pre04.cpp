...
void solve() {
    ...
    dic2.erase(unique(all(dic2)), dic2.end());
    FenwickTree bit(n + 1), bit2(n + 1);
    int ans = 0;
    rep (i, 0, n) {
        int aa = lower_bound(all(dic), a[i]) - dic.begin() + 1,
            bb = lower_bound(all(dic2), b[i]) - dic2.begin() + 1;
        int cur = max(bit.query(aa - 1), bit2.query(bb - 1)) + 1;
        bit.mod(aa, cur);
        bit2.mod(bb, cur);
        chmax(ans, cur);
    }
