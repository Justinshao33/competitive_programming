    ...
    rep (bit, 0, 1 << q) {
        ...
        rep (i, 0, n) {
            ...
            if (w[i] != -1) {
                rep (j, 0, 16) {
                    if (w[i] >> j & 1) {
                        G.add_edge(id(i, j, 1), t, IINF);
                    } else {
                        G.add_edge(s, id(i, j, 0), IINF);
                    }
                }
            }
        }
        rep (k, 0, q) {
            auto &[ty, u, i, v, j] = cons[k];
            int b = (bit >> k & 1);
            if (b) G.add_edge(id(u, i, 1), t, IINF);
            else G.add_edge(s, id(u, i, 0), IINF);
            b ^= ty;
            if (b) G.add_edge(id(v, j, 1), t, IINF);
            else G.add_edge(s, id(v, j, 0), IINF);
        }
        for (auto [u, v] : edge) rep (i, 0, 16) {
            G.add_edge(id(u, i, 0), id(v, i, 1), 1);
            G.add_edge(id(v, i, 0), id(u, i, 1), 1);
