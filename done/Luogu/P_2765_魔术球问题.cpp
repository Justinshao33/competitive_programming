...
void solve() {
    int n; cin >> n;
    Dinic G(MAXN);
    int s = 200001, t = s + 1, cur = 0, cnt = 0;
    vector<int> st(n + 1, 0);
    while (cnt <= n) {
        cur++;
        G.add_edge(s, cur << 1, 1);
        G.add_edge(cur << 1 | 1, t, 1);
        for (int i = sqrt(cur) + 1; i * i - cur < cur; i++) G.add_edge((i * i - cur) << 1, cur << 1 | 1, 1);
        int f = G.flow(s, t);
        if (f == 0) {
            st[cnt] = cur;
            cnt++;
    ...
    rep (i, 0, n) if (!vis[st[i]]) {
        int ptr = st[i];
        while (true) {
            cout << ptr << ' ';
            vis[ptr] = 1;
            int nxt = ptr;
            for (auto [v, r, rc] : G.adj[ptr << 1]) {
                if(v != s && rc == 0) {
                    nxt = (v - 1) >> 1;
                }
            }
