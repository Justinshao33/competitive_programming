...
void solve() {
    int n, m; cin >> n >> m;
    int s = n + m, t = s + 1, tot = 0;
    vector<int> need(n);
    Dinic G(n + m + 2);
    rep (i, 0, n) {
        cin >> need[i];
        G.add_edge(m + i, t, need[i]);
    }
    rep (i, 0, m) {
        G.add_edge(s, i, 1);
        int p; cin >> p;
    ...
    int ans = G.flow(s, t);
    if (ans != accumulate(all(need), 0)) {
        cout << "No solution!\n";
        return;
    }
    vector<vector<int>> typ(n);
    rep (i, 0, m) {
        for (auto [v, r, rc] : G.adj[i]) if (v != s && rc == 0) typ[v - m].pb(i);
    }
    rep (i, 0, n) {
        cout << i + 1 << ": ";
        for (int x : typ[i]) cout << x + 1 << ' ';
        cout << '\n';
    }
}
