...
void solve() {
    int m, n; cin >> m >> n;
    int tot = (m + m + n - 1) * n / 2, cnt = 0;
    MCMF G(2 * tot + 2), G2(2 * tot + 2), G3(2 * tot + 2);
    int s = 2 * tot, t = s + 1;
    vector a(n, vector<int>(m + n));
    rep (i, 0, n) rep (j, 0, m + i) cin >> a[i][j];
    rep (i, 0, m) {
        G.add_edge(s, i, 1, 0);
        G2.add_edge(s, i, 1, 0);
        G3.add_edge(s, i, 1, 0);
    ...
    rep (i, 0, n) {
        rep (j, 0, m + i) {
            if (i == n - 1) {
                G.add_edge(cnt + tot, t, 1, 0);
                G2.add_edge(cnt, t, IINF, -a[i][j]);
                G3.add_edge(cnt, t, IINF, -a[i][j]);
            } else {
                G.add_edge(cnt + tot, nxt, 1, 0);
                G2.add_edge(cnt, nxt, 1, -a[i][j]);
                G3.add_edge(cnt, nxt, IINF, -a[i][j]);
                G.add_edge(cnt + tot, nxt + 1, 1, 0);
                G2.add_edge(cnt, nxt + 1, 1, -a[i][j]);
                G3.add_edge(cnt, nxt + 1, IINF, -a[i][j]);
            }
            G.add_edge(cnt, cnt + tot, 1, -a[i][j]);
            cnt++;
            nxt++;
        }
        nxt++;
