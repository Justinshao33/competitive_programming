...
void solve() {
    ...
    auto solve = [&](auto self, int l, int r, int ql, int qr) -> void {
        ...
        rep (i, 0, sz1) que[ql + i] = q1[i];
        rep (i, 0, sz2) que[ql + sz1 + i] = q2[i];
        self(self, l, mid, ql, ql + sz1 - 1);
        self(self, mid + 1, r, ql + sz1, qr);
    };
    solve(solve, -1e9, 1e9, 0, n + q - 1);
