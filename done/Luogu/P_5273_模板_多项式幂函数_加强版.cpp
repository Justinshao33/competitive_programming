...
struct Poly {
    ...
    Poly pow(int n, i64 k, i64 k2, i64 k3) const {
        int m = 0;
        ...
        mint base = power<mint>(g.freq(0), k2), inv = g.freq(0).inv();
        g = g * inv;
        g = g.log(n - m);
        g = g * k;
        g = g.exp(n - m);
...
void solve() {
    ...
    i64 m = 0, m2 = 0, m3 = 0;
    for (char c : s) {
        m = (m * 10 + c - '0') % 998244353;
        m2 = (m2 * 10 + c - '0') % 998244352;
        m3 = min(1000000000LL, m3 * 10 + c - '0');
    }
    vector<mint> a(n);
    rep (i, 0, n) cin >> a[i].v;
    if (a[0].v == 0 && m3 >= n) {
