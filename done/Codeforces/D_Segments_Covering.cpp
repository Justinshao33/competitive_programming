...
void solve() {
    ...
    rep (i, 0, n) {
        rep (j, 0, 4) cin >> seg[i][j];
    }
    sort(all(seg));
    vector<mint> dp(m + 1, 0), no(m + 1, 1);
    for (auto [l, r, p, q] : seg) {
        mint pr = 1 - (mint(p) * mint(q).inv());
        no[l] *= pr;
    }
    rep (i, 1, m + 1) no[i] *= no[i - 1];
    dp[0] = 1;
    for (auto [l, r, p, q] : seg) {
        mint pr = mint(p) * mint(q).inv();
        dp[r] += dp[l - 1] * pr * (1 - pr).inv() * no[r] * no[l - 1].inv();
    }
    cout << dp[m] << '\n';
}
