...
void solve() {
    ...
    auto calc = [&](int k) -> mint {
        ...
        mint res = 0;
        vector<mint> sum(n + 1); // sum[i] = sum_{j=0}^k Comb(i, j)
        rep (j, 0, k + 1) sum[0] += Comb(0, j);
        rep (i, 1, n + 1) sum[i] = sum[i - 1] * 2 - Comb(i - 1, k);
        rep (l, 0, n) {
            int r = n - 1 - l;
            mint val = Comb(l, k) * sum[r] + sum[l] * Comb(r, k) - Comb(l, k) * Comb(r, k);
            res += catalan(l) * catalan(r) * val;
        }
        return res;
    };
