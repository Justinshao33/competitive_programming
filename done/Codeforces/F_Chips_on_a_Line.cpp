...
void solve() {
    int n, x, m; cin >> n >> x >> m;
    vector<int> fib(2, 1);
    while (ssize(fib) <= 30) fib.push_back(fib.back() + fib.end()[-2]);
    int sz = ssize(fib);
    // 10 * 55 * 1000 * 1000
    int C = fib[x - 1] * n;
    vector dp(n + 1, vector<mint>(C + 1, 0));
    dp[0][0] = 1;
    rep (k, 0, x) {
        rep (i, 1, n + 1) rep (j, max(i, fib[k]), C + 1) {
            dp[i][j] += dp[i - 1][j - fib[k]];
        } 
    }
    ...
    mint ans = 0;
    rep (j, 1, C + 1) if (valid(j)) ans += dp[n][j];
    cout << ans << '\n';
}
