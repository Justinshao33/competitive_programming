...
void solve() {
    ...
    if (a.back() != 0) seg.push_back({0, n - 1});
    sort(all(seg), [&](auto x, auto y) {
        return x.se - x.fi < y.se - y.fi;
    });
    UnionFind dsu(n);
    auto calc = [&](vector<int> &b) -> mint {
        int m = ssize(b);
        ...
        rep (i, 0, m) {
            ...
            rep (j, 1, m + 1) {
                if (b[i]) {
                    ...
                    //     g[j] += dp[k];
                    // }
                    g[j] += dp.back() - (j - 2 >= 0 ? dp[j - 2] : 0);
                } else {
                    g[j] += dp[j - 1] - (j - 2 >= 0 ? dp[j - 2] : 0);
                }
            }
            swap(dp, g);
