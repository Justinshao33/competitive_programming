...
info op(info a, info b) {
    return {a.pro * b.pro, a.msk | b.msk, a.len + b.len};
}
...
info mapp(func f, info a) {
    return {f.mul.pow(a.len) * a.pro, a.msk | f.msk, a.len};
}

func comp(func a, func b) { // new, old
    return {a.mul * b.mul, a.msk | b.msk};
}
...
void solve() {
    ...
    rep (i, 2, 301) {
        bool f = 0;
        for (int j = 2; j * j <= i; j++) if (i % j == 0) f = 1;
    ...
    rep (i, 0, n) {
        int b; cin >> b;
        int msk = 0;
        rep (j, 0, ssize(prime)) if (b % prime[j] == 0) msk |= 1LL << j;
        a.push_back({b, msk, 1});
    }
    lazy_segtree<info, op, e, func, mapp, comp, id> tree(a);
    vector<mint> mul(ssize(prime));
    rep (i, 0, ssize(prime)) mul[i] = 1 - mint(prime[i]).inv();
    while (q--) {
        string s; cin >> s;
        int l, r; cin >> l >> r;
        l--;
        if (s[0] == 'T') {
            auto [p, m, _] = tree.prod(l, r);
            rep (i, 0, ssize(prime)) if (m >> i & 1) {
                p *= mul[i];
            }
            cout << p << '\n';
        } else {
            int x; cin >> x;
            int msk = 0;
            rep (i, 0, ssize(prime)) if (x % prime[i] == 0) msk |= 1LL << i;
            tree.apply(l, r, {x, msk});
        }
    }
}
