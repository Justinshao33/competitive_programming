...
void solve() {
    ...
    auto dic = a;
    sort(all(dic));
    Fenwick<int> cnt(n), bit(n);
    rep (i, 0, n) {
        int pos = lower_bound(all(dic), a[i]) - dic.begin();
        // debug(pos);
        cnt.add(pos, 1);
        bit.add(pos, a[i]);
        if (i < 2) continue;
        int l = 1, r = i + 2;
        auto get = [&](int x) -> int {
            auto [p, _] = cnt.select(x);
            return bit.qry(p, p + 1);
        };
        auto L = [&](int x) -> int {
            if (x - 2 <= 0) return 0;
            auto [p, _] = cnt.select(x - 2);
            return (x - 2) * get(x - 1) - bit.qry(p);
        };
        auto R = [&](int x) -> int {
            if (cnt.qry(0, n) < x + 2) return 0;
            auto [p, _] = cnt.select(x + 2);
            return bit.qry(p, n) - get(x + 1) * (i + 1 - (x + 2) + 1);
        };
        ...
        int val = max(L(l), R(l));
        if (l + 1 <= i + 1) {
            val = min(val, max(L(l + 1), R(l + 1)));
        }
        // debug(val);
        cout << mint(i - 1).inv() * val << '\n';
    }
}
