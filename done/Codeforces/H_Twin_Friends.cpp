...
void solve() {
    ...
    for (char c : t) cntb[c - 'A']++;
    vector dp(27, vector<mint>(n + 1, 0));
    dp[26][0] = 1;
    int sum = 0;
    for (int i = 25; i >= 0; --i) {
        sum += cnta[i];
        rep (j, 1, n + 1) dp[i + 1][j] += dp[i + 1][j - 1];
        rep (j, 0, (i ? cnta[i - 1] : 0) + 1) {
            dp[i][sum + j] = dp[i + 1][sum] - (sum + j - cntb[i] <= 0 ? 0 : dp[i + 1][sum + j - cntb[i] - 1]);
            // rep (k, max(0, sum + j - cntb[i]), sum + 1) {
            //     dp[i][sum + j] += dp[i + 1][k];
            // }
            dp[i][sum + j] *= Comb((i ? cnta[i - 1] : 1), j);
        }
    }
    dp[0][n] *= fac[n];
    rep (i, 0, 26) dp[0][n] *= inv[cnta[i]];
    cout << dp[0][n] << '\n';
}
