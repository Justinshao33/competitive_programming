...
void solve() {
    ...
    rep (i, 0, n) {
        // cout << L[i] << ' ' << R[i] << '\n';
        // ans += mint(a[i]).inv() * (i - L[i]) * (R[i] - i);
        ans += mint(a[i]).inv() * (i + 1) * (n - i);
        sum += mint(a[i]).inv() * (i - L[i]) * (R[i] - i);
    }
    ans -= sum;
    // (A + 1) / A ? B / (B - 1)
    // B - A ? 1
    ...
    while (m--) {
        int k; cin >> k;
        while (ptr < n && 1 + k >= a[p[ptr]]) {
            int x = p[ptr];
            sum -= mint(a[x]).inv() * (x - L[x]) * (R[x] - x);
            // k - a[x] + 2
            cnt += (x - L[x]) * (R[x] - x);
            sum2 += (x - L[x]) * (R[x] - x) * (2 - a[x]);
            ptr++;
        }
        cout << ans + sum * (k + 1) + sum2 + k * cnt << '\n';
    }
}
