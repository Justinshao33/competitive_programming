...
void solve() {
    ...
    vector<mint> p2(n + q + 1);
    p2[0] = 1;
    rep (i, 1, n + q + 1) p2[i] = p2[i - 1] * 2;
    while (q--) {
        int t, x; cin >> t >> x;
        if (t == 1) {
            str[x]++;
            ...
            vector<int> bdd(C, 0);
            for (int i = C - 1, occ = 0; i >= 0; --i) {
                if (x >> i & 1) {
                    bdd[occ + i] += 1;
                    occ++;
                }
            }
            vector<mint> dp(2, 0); // 0: =0, 1: >0
            dp[0] = 1;
            for (int i = C - 1; i >= 0; --i) {
                mint val = p2[str[i]];
                dp[1] *= val;
                rep (j, 0, bdd[i] + 1) val -= Comb(str[i], j); // sum (bdd + 1) <= 60 + 60
                dp[1] += dp[0] * val;
                dp[0] *= Comb(str[i], bdd[i]);
            }
            cout << dp[0] + dp[1] << '\n';    
        }
    }
