...
void solve() {
    ...
    vector<int> leaf;
    rep (i, 1, n) if (ssize(adj[i]) == 1) leaf.pb(i);
    dfs(dfs, 0, -1);
    int mn = min(dep[leaf[0]] - dep[lca], dep[leaf[1]] - dep[lca]);
    if (dep[leaf[0]] - dep[lca] == dep[leaf[1]] - dep[lca]) cout << mint(2).pow(n - mn * 2) * 2 << '\n';
    else cout << mint(2).pow(n - mn * 2) + mint(2).pow(n - mn * 2 - 1) << '\n';
}
