...
void solve() {
    ...
    vector dp(n, vector (k + 1, vector<mint>(2))); // 0 invalid tree furthest white, 1 valid tree closest black 
    auto dfs = [&](auto self, int u, int pa) -> void {
        dp[u][0][0] = dp[u][0][1] = 1;
        for (int v : adj[u]) {
            if (v == pa) continue;
            self(self, v, u);
            vector g(k + 1, vector<mint>(2));
            rep (i, 0, k + 1) rep (j, 0, k + 1) {
                if (max(i, j + 1) <= k) g[max(i, j + 1)][0] += dp[u][i][0] * dp[v][j][0];
                g[min(i, j + 1)][1] += dp[u][i][1] * dp[v][j][1];
                if (i + j + 1 <= k) {
                    g[j + 1][1] += dp[u][i][0] * dp[v][j][1];
                    g[i][1] += dp[u][i][1] * dp[v][j][0];
                } else {
                    g[i][0] += dp[u][i][0] * dp[v][j][1];
                    if (j + 1 <= k) g[j + 1][0] += dp[u][i][1] * dp[v][j][0];
                }
            }
            swap(dp[u], g);
        }
