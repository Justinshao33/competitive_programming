...
void solve() {
    ...
    while (q--) {
        ...
        Line l(a, b);
        // debug(a, b);
        auto cut = gon.intersect(l);
        bool f = cut.empty();
        for (int id : cut) {
            if (!isInter(l, {gon.A[id], gon.A[(id + 1) % n]})) 
                f = true;
        }
        if (f) {
            cout << abs(a - b) << '\n';
        ...
        Real ans = 9e18;
        auto A = gon.tangent2(a);
        auto B = gon.tangent2(b);
        rep (i, 0, 2) {
            auto v1 = gon.A[A[i]] - a, v2 = gon.A[B[i ^ 1]] - b;
            if ((v1 ^ v2) == 0) continue;
            Pt p = LineInter({a, gon.A[A[i]]}, {b, gon.A[B[i ^ 1]]});
            chmin(ans, abs(p - a) + abs(b - p));
        }
        if (fabs(ans - 9e18) < eps<Real>) cout << -1 << '\n';
        else cout << ans << '\n';
    }
}
