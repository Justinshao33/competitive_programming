...
void solve() {
    ...
    auto calc = [&](vector<vector<int>> &G, vector<vector<int>> &H) -> mint {
        int n = ssize(G), m = ssize(H);
        vector dp(n, vector<mint>(m, 0));
        vector vis(n, vector<bool>(m, 0));
        auto dfs = [&](auto self, int u, int p, int u2, int p2) -> mint {
            if (vis[u][u2]) return dp[u][u2];
            vis[u][u2] = 1;
            int ch = ssize(H[u2]) - (p2 != -1);
            vector<mint> g(1 << ch);
            g[0] = 1;
            for (int v : G[u]) {
                if (v == p) continue;
                ...
                for (int v2 : H[u2]) {
                    if (v2 == p2) continue;
                    val.pb(self(self, v, u, v2, u2));
                }
                for (int bit = (1 << ch) - 1; bit >= 0; --bit) {
                    rep (i, 0, ch) if (bit >> i & 1 ^ 1) {
                        g[bit | (1 << i)] += g[bit] * val[i];
                    }
                }
            }
            return dp[u][u2] = g[(1 << ch) - 1];
        };
        mint ans = 0;
        rep (i, 0, m) {
            rep (j, 0, n) rep (k, 0, m) {
