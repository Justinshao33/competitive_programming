...
void solve() {
    ...
    vector L(n + 2, vector<mint>(n + 2, 0)); // edge to right
    auto R = L; // edge to left
    L[0][0] = 1;
    rep (i, 1, n + 1) {
        rep (j, 0, i + 1) {
            L[i][j] += L[i - 1][j] * j; // in->pre_in or pre_out->out
            if (s[i] == '<') L[i][j] += L[i - 1][j + 1] * (j + 1) * j; // delete a out edge (j + 1) and go to a in edge (j)
            else if (j > 0) L[i][j] += L[i - 1][j - 1]; // head
        }
    }
    R[n + 1][0] = 1;
    for (int i = n; i > 0; --i) {
        rep (j, 0, (n - i + 1) + 1) {
            R[i][j] += R[i + 1][j] * j;
            if (s[i] == '>') R[i][j] += R[i + 1][j + 1] * (j + 1) * j;
            else if (j > 0) R[i][j] += R[i + 1][j - 1];
        }
    }
    rep (i, 1, n + 1) {
        if (i == 1) {
            ...
            rep (j, 0, n + 1) {
                if (j + 1 <= n) {
                    ans += L[i - 1][j] * fac[j] * R[i + 1][j + 1] * fac[j + 1]; // permute the way to connect
                    ans += L[i - 1][j + 1] * fac[j + 1] * R[i + 1][j] * fac[j];
                }
                ans += L[i - 1][j] * fac[j] * R[i + 1][j] * fac[j] * 2; // L -> i or i <- R 
            }
            cout << ans << ' ';
        }
    }
