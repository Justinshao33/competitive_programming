...
void solve() {
    ...
    vector ok(n, vector<int>(n, 0));
    rep (i, 0, m) {
        int a, b; cin >> a >> b;
        a--, b--;
        ok[a][b] = ok[b][a] = 1;
    }
    vector<mint> dp(1 << n, 0); // # cactus
    dp[0] = 1;
    mint inv = mint(2).inv();
    auto calc = [&](int s) -> void {
        vector cyc(n, vector<mint>(1 << n, 0)); // # cycle starting from s
        cyc[s][1 << s] = 1;
        rep (bit, 0, 1 << n) rep (i, s, n) if (bit >> i & 1) {
            rep (j, s, n) if (ok[i][j] && (bit >> j & 1)) {
                cyc[j][bit] += cyc[i][bit ^ (1 << j)];
            }
        }
        rep (bit, 0, 1 << n) rep (i, 0, n) if ((bit >> i & 1) && (bit >> s & 1) && ok[i][s]) {
            if (__builtin_popcount(bit) != 2) cyc[i][bit] *= inv; 
            dp[bit] += cyc[i][bit];
        }
    };
    rep (i, 0, n) calc(i);
    ...
    rep (x, 0, n) {
        vector<mint> g(1 << n, 0);
        rep (bit, 0, 1 << n) { // first cactus S
            g[bit] += dp[bit];
            if (bit >> x & 1 ^ 1) continue;
            int comp = full ^ bit;
            for (int mask = comp; mask > 0; mask = (mask - 1) & comp) { // second cactus T
                // S \inter T = {x} 
                int mask2 = mask ^ (1 << x);
                g[bit | mask2] += dp[bit] * g[mask2];
            }
        }
        swap(dp, g);
    }
