...
void solve() {
    ...
    dfs(dfs, 0, -1);
    vector dp(n, vector<mint>(n)); // begin at (i, j)
    auto dp2 = dp; // begin at (i's subtree, j'subtree)
    for (int x : ord) for (int y : ord) {
        if (s[x] == s[y]) {
            for (int u : adj[x]) if (u != fa[x]) for (int v : adj[y]) if (v != fa[y]) {
                dp[x][y] += dp2[u][v];
            }
            dp[x][y]++;
        }
        for (int u : adj[x]) if (u != fa[x]) dp2[x][y] += dp2[u][y];
        for (int v : adj[y]) if (v != fa[y]) dp2[x][y] += dp2[x][v];
        for (int u : adj[x]) if (u != fa[x]) for (int v : adj[y]) if (v != fa[y]) {
            dp2[x][y] -= dp2[u][v];
        }
        dp2[x][y] += dp[x][y];
    }
    rep (i, 0, n) cout << dp2[i][i] << " \n" [i == n - 1];
}
