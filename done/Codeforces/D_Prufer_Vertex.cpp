...
void solve() {
    ...
    dfs(dfs, n - 1, -1);
    set<int> anc;
    for (int cur = n - 2; cur != -1; cur = par[cur]) anc.insert(cur); 
    int cc = n - m;
    mint oans = cc - 2 < 0 ? mint(n).inv() : mint(n).pow(cc - 2);
    rep (i, 0, n) if (dsu.find(i) == i) oans *= dsu.size(i);
    rep (i, 0, n - 1) {
        if (dsu.same(i, n - 1) && !mp.contains({i, n - 1})) {
            cout << 0 << " \n" [i == n - 2];
        } else if (dsu.same(n - 2, n - 1) && !anc.contains(i)) {
            cout << 0 << " \n" [i == n - 2];
        } else if (dsu.same(n - 2, n - 1)) {
            cout << oans << " \n" [i == n - 2];
        } else if (dsu.same(i, n - 1)) {
            cout << oans * sz[i] / sz[n - 1] << " \n" [i == n - 2];
        } else {
            // oans = n^{cc-2} * \prod s_i
            // ans = oans / (sz[i] * sz[n - 1]) * (sz[i] + sz[n - 1]) / n * sz[i] / (sz[i] + sz[n - 1]);
            // ans = oans / sz[n - 1] / n
            mint ans = oans / (dsu.size(i) * dsu.size(n - 1)) * (dsu.size(i) + dsu.size(n - 1)) / n;
            if (!dsu.same(i, n - 2)) ans *= mint(dsu.size(i)) / (dsu.size(i) + dsu.size(n - 1));
            cout << ans << " \n" [i == n - 2];
        }
    }
}
