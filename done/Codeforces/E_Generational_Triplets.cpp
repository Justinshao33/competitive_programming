...
void solve() {
    ...
    // a < c
    int n; cin >> n;
    vector dp(2, vector(2, vector<mint>(2))); // c < n, a < c, next bit & = 1?
    dp[0][0][0] = 1;
    for (int i = 59; i >= 0; --i) {
        vector g(2, vector(2, vector<mint>(2)));
        int nb = n >> i & 1;
        rep (j, 0, 2) rep (k, 0, 2) rep (l, 0, 2) {
            rep (cb, 0, 2) rep (ab, 0, 2) {
                if (l ^ (cb + ab == 2)) continue;
                if (!j && cb > nb) continue;
                if (!k && ab > cb) continue;
                int nj, nk, nl;
                nj = j | (cb < nb);
