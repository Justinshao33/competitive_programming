...
void solve() {
    ...
    dfs(dfs, 0);
    auto dfs2 = [&](auto self, int u, mint dpf) -> void {
        ans[u] = dp[u] * (dpf + 1);
        vector<mint> pre{dpf + 1}, suf;
        for (int v : adj[u]) {
            pre.push_back(pre.back() * (dp[v] + 1));
        }
        reverse(all(adj[u]));
        for (int v : adj[u]) {
            suf.push_back(suf.empty() ? (dp[v] + 1) : suf.back() * (dp[v] + 1));
        }
        suf.push_back(suf.empty() ? dpf : suf.back() * dpf);
        reverse(all(adj[u]));
        reverse(all(suf));
        rep (i, 0, ssize(adj[u])) {
            int v = adj[u][i];
            mint val = pre[i] * (i + 2 < ssize(suf) ? suf[i + 2] : 1);
            self(self, v, val);
        }
    };
    ans[0] = dp[0];
