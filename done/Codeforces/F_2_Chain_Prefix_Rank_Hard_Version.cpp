...
void solve() {
    ...
    vector<map<int, pair<mint, int>>> G(n + 1);
    // {dp, #node on edge}
    auto add_edge = [&](int x, int y) -> void {
        G[x][y] = G[y][x] = {1, 0};
    };
    Treap *ord = merge(new Treap(0), new Treap(n));
    auto dfs = [&](auto self, int u) -> void {
        {
            Treap *l, *r;
            split2(ord, l, r, a[u]);
            ord = merge(merge(l, new Treap(u)), r);
        }
        {
            int L = kth(ord, a[u])->key, R = kth(ord, a[u] + 2)->key;
            if (G[L].count(R)) {
                G[L].erase(R);
                G[R].erase(L);
            }
        ...
        {
            auto [l, rem, r] = interval(ord, a[u] + 1, a[u] + 1);
            delete rem;
            ord = merge(l, r);
        }
    };
    dfs(dfs, 1);
    queue<int> q;
    rep (i, 1, n) if (ssize(G[i]) <= 2) q.push(i);
    while (!q.empty()) {
        int u = q.front(); q.pop();
        int deg = ssize(G[u]);
        if (deg == 0) continue;
        ...
        if (deg == 1) {
            ...
            mint f = f1 * f2; int g = g1 + g2 + 1;
            if (G[v].contains(w)) {
                auto [f3, g3] = G[v][w];
                G[v][w] = G[w][v] = { f * f3 * Comb(g + g3, g) , g + g3 };
            } else {
                G[v][w] = G[w][v] = { f, g };
            }
        }
        for (auto &p : G[u]) if (p.fi != 0 && p.fi != n && ssize(G[p.fi]) <= 2) q.push(p.fi);
        G[u].clear();
    }
    cout << G[0][n].fi << '\n';
}
