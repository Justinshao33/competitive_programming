...
using mint = atcoder_modint::modint1000000007;
...
void solve() {
    ...
    int ox, oy;
    rep (i, 0, 3) rep (j, 0, n) {
        if (g[i][j] == 'O') ox = i, oy = j;
    }
    vector<array<int, 4>> cand;
    if (ox == 0 && g[ox + 1][oy] != 'X' && g[ox + 2][oy] != 'X') cand.pb({ox + 1, oy, ox + 2, oy});
    if (ox == 2 && g[ox - 1][oy] != 'X' && g[ox - 2][oy] != 'X') cand.pb({ox - 2, oy, ox - 1, oy});
    if (oy >= 2 && g[ox][oy - 1] != 'X' && g[ox][oy - 2] != 'X') cand.pb({ox, oy - 2, ox, oy - 1});
    if (oy + 2 < n && g[ox][oy + 1] != 'X' && g[ox][oy + 2] != 'X') cand.pb({ox, oy + 1, ox, oy + 2});
    int m = ssize(cand);
    mint ans = 0;
    rep (t, 1, 1 << m) {
        g[ox][oy] = 'X';
        rep (i, 0, m) if (t >> i & 1) {
            g[cand[i][0]][cand[i][1]] = 'X';
            g[cand[i][2]][cand[i][3]] = 'X';
        }
        vector<mint> dp(8, 0);
        rep (i, 0, 8) {
            bool no = 0;
            rep (j, 0, 3) if ((i >> j & 1 ^ 1) && g[j][0] == 'X') no = 1;
            if (no) continue;
            int mask = 0;
            rep (j, 0, 3) if (g[j][0] == 'X') mask |= 1 << j;
            int mm = i ^ mask;
            if (mm == 1 || mm == 2 || mm == 4 || mm == 5 || mm == 7) continue;
            dp[i] = 1;
        }
        // cout << dp;
        rep (i, 1, n) {
            ...
            rep (bi, 0, 8) {
                rep (bj, 0, 8) {
                    ...
                    rep (j, 0, 3) {
                        if (((bj >> j & 1 ^ 1) && g[j][i] == 'X') || 
                            ((bi >> j & 1 ^ 1) && (g[j][i] == 'X' || (bj >> j & 1 ^ 1)))) no = 1;
                    }
                    if (no) continue;
                    int mask = 0;
                    rep (j, 0, 3) if (g[j][i] == 'X' || (bi >> j & 1 ^ 1)) mask |= 1 << j;
                    int mm = bj ^ mask;
                    if (mm == 1 || mm == 2 || mm == 4 || mm == 5 || mm == 7) continue;
                    dp2[bj] += dp[bi];  
                }
        ...
        mint res = dp[7];
        if (__builtin_popcount(t) & 1) ans += res;
        else ans -= res;
        // cout << res << '\n';
        g = og;
    }
