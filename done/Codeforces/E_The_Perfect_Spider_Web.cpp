...
void solve() {
    ...
    auto calc2 = [&](Pt p) -> Real {
        Real ans = 0;
        rep (i, 0, n) {
            Real A = fabs((pt[i] - p) ^ (pt[(i + 1) % n] - p));
            if (A - ans > eps<Real>) ans = A;
        }
        return ans / 2;
    };
    auto calc = [&](Real x) -> Real {
        auto v = CV.intersect(Line{Pt{x, 0}, Pt{x, 1}});
        Real l = 1e6, r = 0;
        for (int id : v) {
            Pt inter = LineInter(Line{CV.A[id], CV.A[(id + 1) % n]}, Line{Pt{x, 0}, Pt{x, 1}});
            chmin(l, inter.y);
            chmax(r, inter.y);
        }
        rep (t, 0, 200) {
            Real ml = l + (r - l) / 3, mr = r - (r - l) / 3;
            Pt lp{x, ml}, rp{x, mr};
            Real L = calc2(lp), R = calc2(rp);
            if (L < R) r = mr;
            else l = ml;
        }
        return calc2({x, l});
