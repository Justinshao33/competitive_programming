...
struct FenwickTree {
    vector<pair<int, mint>> BIT;
    FenwickTree(int n) : BIT(n + 1, {IINF, 0}) {}

    void mod(int x, pair<int, mint> val) {
        while(x < BIT.size()){
            if (BIT[x].fi > val.fi) {
                BIT[x] = val;
    ...
    pair<int, mint> query(int x) {
        pair<int, mint> res = {IINF, 0};
        while (x > 0) {
            if (res.fi > BIT[x].fi) {
                res = BIT[x];
            } else if (res.fi == BIT[x].fi) {
                res.se += BIT[x].se;
            }
            x -= x & -x;
        }
    ...
    // pair<int, mint> query(int l, int r) {
    //     if (l > r) return {0, 0};
...
void solve() {
    ...
    FenwickTree bit(2 * n);
    vector<pair<int, mint>> dp(n);
    rep (i, 0, n) {
        int id = lower_bound(all(dic), doll[i].se) - dic.begin() + 1;
        dp[i] = bit.query(id);
        if (dp[i].fi == IINF) dp[i] = {doll[i].se - doll[i].fi, 1};
        else dp[i].fi += doll[i].se - doll[i].fi;
        id = lower_bound(all(dic), doll[i].fi) - dic.begin() + 1;
        bit.mod(id, dp[i]);
    }
    pair<int, mint> ans = {IINF, 0};
    int mxl = 0;
    for (int i = n - 1; i >= 0; --i) {
        chmax(mxl, doll[i].se);
        if (doll[i].fi <= mxl) continue;
        dp[i].fi += doll[i].fi;
        if (ans.fi > dp[i].fi) {
            ans = dp[i];
        } else if (ans.fi == dp[i].fi) {
            ans.se += dp[i].se;
        }
    }
    cout << ans.se << '\n';
