...
mint dp[1005][31][31][31][2];

void solve() {
    mint::set_mod(1000'000'009);
    int n, h; cin >> n >> h;
    dp[0][1][1][1][1] = 4;
    rep (t, 0, n - 1) {
        rep (i, 0, h + 1) rep (j, 0, h + 1) rep (k, 0, h + 1) rep (l, 0, 2) {
            int ni = min(h, i + 1), nj = min(h, j + 1), nk = min(h, k + 1), nl = (l ? 1 : h);
            dp[t + 1][ni][nj][nk][l] += dp[t][i][j][k][l];
            dp[t + 1][nl][nj][nk][i < h] += dp[t][i][j][k][l];
            dp[t + 1][ni][nl][nk][j < h] += dp[t][i][j][k][l];
            dp[t + 1][ni][nj][nl][k < h] += dp[t][i][j][k][l];
        }
    }
    mint ans = 0;
    rep (i, 0, h + 1) rep (j, 0, h + 1) rep (k, 0, h + 1) rep (l, 0, 2) {
        if (i < h || j < h || k < h || l) ans += dp[n - 1][i][j][k][l];
    } 
    cout << ans << '\n';
}
