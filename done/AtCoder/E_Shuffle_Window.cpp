...
void solve() {
    ...
    mint inv = (k - 1) * mint(k).inv();
    mint two = mint(2).inv();
    FenwickTree bit(n + 1), bit2(n + 1);
    mint ans = 0;
    rep (i, 0, n) {
        int p = max(0, i + 1 - k);
        mint vl = bit.query(a[i]), vr = bit.query(n) - vl;
        ans += vl * inv.pow(p) * two;
        ans += (i - bit2.query(a[i])) - vr * inv.pow(p) * two;
        bit.mod(a[i], mint(1 / inv).pow(p));
        bit2.mod(a[i], 1);
    }
    cout << ans.val() << '\n';
}
