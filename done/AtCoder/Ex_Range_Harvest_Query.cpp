...
info mapping(func f, info a) {
    return {
        a.h,
        a.len,
        a.sum * f.a + a.len * (2 * a.h * f.b + (a.len - 1) * f.b) * inv
    };
}
...
void solve() {
    ...
    vector<info> a;
    rep (i, 0, n) {
        a.pb({dic[i], dic[i + 1] - dic[i], 0});
    }
    lazy_segtree<info, op, e, func, mapping, composition, id> seg(a);
    ll pre = 0;
    rep (i, 0, q) {
        seg.apply(0, n, {1, d[i] - pre});
        l[i] = lower_bound(all(dic), l[i]) - dic.begin();
        r[i] = lower_bound(all(dic), r[i] + 1) - dic.begin();
        cout << seg.prod(l[i], r[i]).sum.val() << '\n';
        seg.apply(l[i], r[i], {0, 0});
        pre = d[i];
    }
}
