...
void solve() {
    int n, pr; cin >> n >> pr;
    init(n);
    mint p = pr * mint(100).inv();
    vector<mint> f(n, 0);
    f[0] = 1;
    // x + (1 - p)(x / i) = x(i + 1 - p)/i
    rep (i, 1, n) f[i] = f[i - 1] * (n - i + 1 - p) * mint(n - i).inv();
    /*
    for a fixed x = 2~n:
    Ex = \sum_{i=x-1}^{n-1} C(i-1, x-2) * p^{x-1} * (1-p)^{i-x+1} * f[n-i-1]
       = p^{x-1} * \sum_{i=0}^{n-x} C(i+x-2, x-2) * (1-p)^i * f[n-i-x]
       = p^{x-1}/(x-2)! \sum_{i=0}^{n-x} (i+x-2)!/i! * (1-p)^i * f[n-i-x]
    Let F(x) = ((1-p)^i/i!)x^i, G(x) = ((n-i-2)!f[i])x^i
    Ei = (p^{i-1}/(i-2)!)[x^{n-i}]F(x)G(x) 
    */
    vector<mint> F(n), G(n);
    rep (i, 0, n - 1) {
        F[i] = mint(1 - p).pow(i) * inv[i];
        G[i] = fac[n - i - 2] * f[i];
    }
    auto H = convolution(F, G);
    cout << f[n - 1] << '\n';
    rep (i, 2, n + 1) cout << p.pow(i - 1) * inv[i - 2] * H[n - i] << '\n';
}
