...
void powpow(Mat &res, ll exp) { // exp = exp - 1
    Mat base = res;
    while (exp > 0) {
...
Mat powpow2(Mat base, ll exp) { // exp = exp
...
void solve() {
    ...
    cin >> a[n + 1];
    rep (i, 1, n + 1) cin >> a[i];
    // place some prefix in [0, S] (begin with 0 and end with S) that no element is in {a}
    // maintain previous maximum placed number's parity
    // odd means that every adjcent elements must have different parities
    // if not choose ==> dp[i][0] -> dp[i + 1][1], dp[i][1] -> dp[i + 1][0] (swap)
    // if choose (not in {a}) ==> dp[i][0] -> dp[i + 1][0]
    // so for the number not in {a} we can just use matrix exp {{1, 1}, {1, 0}}
    // for the number in {a} just simply swap the dp
    // cuz we must choose S, so i think the answer is dp[S & 1]? (nah is dp[1] cuz i've already swap)
    vector<mint> dp(2, 0);
    dp[0] = 1; // 0 must place
    Mat M(2, 2);
    M.a = {{1, 1}, {1, 0}};
    rep (i, 1, n + 2) {
        auto tran = powpow2(M, a[i] - a[i - 1] - 1);
        vector<mint> g(2);
        g[0] = dp[0] * tran.a[0][0] + dp[1] * tran.a[0][1];
        g[1] = dp[0] * tran.a[0][1] + dp[1] * tran.a[1][1];
        swap(dp, g);
        swap(dp[0], dp[1]);
        // debug(a[i], dp[0], dp[1]);
    }
