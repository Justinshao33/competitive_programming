...
void solve() {
    ...
    int m = ssize(dic);
    // #all - #not choose color c
    // \sum {C(n, k) - C(n - #c_i, k)}
    // we only care the # of #c_i
    // let b_{c_i} = # of #c_i
    // N = \sum i * b_i
    // => b_i has only \sqrt(n) term
    //    \sum {C(n, k) - C(n - #c_i, k)}
    // =  C(n, k) * m - \sum b_i C(n - i, k)
    // O(N \sqrt(N))
    vector<int> cnt(m, 0);
    rep (i, 0, n) cnt[lower_bound(all(dic), c[i]) - dic.begin()]++;
    vector<int> b(n + 1, 0);
    rep (i, 0, m) b[cnt[i]]++;
    vector<pii> b2;
    rep (i, 0, n + 1) if (b[i]) b2.pb({i, b[i]});
    rep (k, 1, n + 1) {
        mint ans = Comb(n, k) * m;
        for (auto &[i, bi] : b2) ans -= Comb(n - i, k) * bi;
        cout << ans * Comb(n, k).inv() << '\n';
    }
}
