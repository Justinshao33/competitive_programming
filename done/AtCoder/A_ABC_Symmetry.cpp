...
void solve() {
    ...
    s = '$' + s;
    dp[0][0][0][0][0] = 1;
    rep (i, 1, n + 1) rep (a, 0, i + 1) rep (b, 0, i + 1) rep (c, 0, i + 1) {
        if (i - a - b - c < 0) continue;
        rep (x, 0, 4) {
            if (s[i] == 'A' || s[i] == '?') {
                int msk = x ^ 1, na = a, nb = b, nc = c;
                if (msk == 1) na++;
                else if (msk == 2) nb++;
                else if (msk == 3) nc++;
                dp[msk][i][na][nb][nc] += dp[x][i - 1][a][b][c];
            }
            if (s[i] == 'B' || s[i] == '?') {
                int msk = x ^ 2, na = a, nb = b, nc = c;
                if (msk == 1) na++;
                else if (msk == 2) nb++;
                else if (msk == 3) nc++;
                dp[msk][i][na][nb][nc] += dp[x][i - 1][a][b][c];
    ...
    rep (a, 0, n + 1) rep (b, 0, n + 1) rep (c, 0, n + 1) {
        int d = n + 1 - a - b - c;
        if (d < 0) continue;
        if (calc(a) + calc(b) + calc(c) + calc(d) < k) continue;
        rep (x, 0, 4) {
            ans += dp[x][n][a][b][c];
        }
    }
