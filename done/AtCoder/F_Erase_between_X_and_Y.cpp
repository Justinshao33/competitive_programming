...
struct Treap {
    ...
    int key, size;
    ll sum, val;
    Treap(int k, ll v) : l(nullptr), r(nullptr), fa(nullptr), key(k), size(1), sum(v), val(v) {}
    void pull();
    void push() {};
};
...
void solve() {
    int q; cin >> q;
    Treap *rt = nullptr;
    vector<Treap*> node(q + 1, nullptr);
    node[0] = new Treap(1, 0);
    rt = merge(rt, node[0]);
    rep (i, 1, q + 1) {
        int t, x; cin >> t >> x;
        if (t == 1) {
            node[i] = new Treap(1, i);
            int sz = get_pos(node[x]);
            Treap *a, *b;
            split2(rt, a, b, sz);
            rt = merge(merge(a, node[i]), b);
        } else {
            int y; cin >> y;
            int sz = get_pos(node[x]), sz2 = get_pos(node[y]);
            if (sz > sz2) swap(sz, sz2);
            if (sz + 1 > sz2 - 1) {
                cout << 0 << '\n';
                continue;
            }
            auto [a, b, c] = interval(rt, sz + 1, sz2 - 1);
            cout << SUM(b) << '\n';
            rt = merge(a, c);
        }
    }
