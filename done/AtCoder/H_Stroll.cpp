...
void solve() {
    int n, m, T; cin >> n >> m >> T;
    vector<pii> edge(m);
    vector p(m, vector<int>(T + 1));
    rep (i, 0, m) {
        cin >> edge[i].F >> edge[i].S;
        edge[i] = edge[i] - make_pair(1, 1);
        rep (j, 1, T + 1) cin >> p[i][j];
    }
    vector dp(n, vector<mint>(T + 1, 0));
    auto dc = [&](auto self, int l, int r) -> void {
        ...
        rep (i, 0, m) {
            auto [a, b] = edge[i];
            vector<mint> A, B;
            rep (j, l, mid) A.pb(dp[a][j]);
            rep (j, 0, r - l) B.pb(p[i][j]);
            auto c = convolution(A, B);
            rep (j, mid, r) dp[b][j] += c[j - l];
            A.clear();
            B.clear();
            rep (j, l, mid) A.pb(dp[b][j]);
            rep (j, 0, r - l) B.pb(p[i][j]);
            auto d = convolution(A, B);
            rep (j, mid, r) dp[a][j] += d[j - l];
        }
        self(self, mid, r);
    };
    dp[0][0] = 1;
