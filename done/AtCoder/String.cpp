...
void solve() {
    ...
    rep (i, 0, 3) {
        cin >> str[i];
        for (char &c : str[i]) c -= 'a';
    }
    vector dp(ssize(str[0]), vector (ssize(str[1]), vector<mint>(ssize(str[2]))));
    dp[0][0][0] = 1;
    rep (t, 0, n) {
        vector g(ssize(str[0]), vector (ssize(str[1]), vector<mint>(ssize(str[2]))));
        rep (i, 0, ssize(str[0])) {
            rep (j, 0, ssize(str[1])) {
                rep (k, 0, ssize(str[2])) {
                    rep (l, 0, 26) {
                        int ni = i + (l == str[0][i]), nj = j + (l == str[1][j]), nk = k + (l == str[2][k]);
                        if (ni >= ssize(str[0]) || nj >= ssize(str[1]) || nk >= ssize(str[2])) continue;
                        g[ni][nj][nk] += dp[i][j][k];
                    }
                }
            }
