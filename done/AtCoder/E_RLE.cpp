...
void solve() {
    int n, p; cin >> n >> p;
    mint::set_mod(p);
    vector dp(n + 1, vector<mint>(n + 1, 0)), sum(n + 1, vector<mint>(n + 1, 0));
    vector<int> ten(5);
    ten[0] = 1;
    rep (i, 1, 5) ten[i] = ten[i - 1] * 10;
    dp[0][0] = mint(25).inv() * 26;
    rep (i, 1, n + 1) sum[i][0] = dp[0][0];
    rep (j, 1, n + 1) rep (i, 1, n + 1) {
        rep (k, 1, 5) {
            if (j - k - 1 < 0) continue;
            int l = max(0, i - ten[k] + 1), r = max(0, i - ten[k - 1] + 1);
            dp[i][j] += 25 * (sum[r][j - k - 1] - sum[l][j - k - 1]);
        }
