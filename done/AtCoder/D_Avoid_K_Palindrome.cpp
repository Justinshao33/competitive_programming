...
void solve() {
    ...
    rep (i, 1, k) {
        ...
        rep (bit, 0, 1 << k) {
            int nxt = (bit << 1) & ((1 << k) - 1);
            if (s[i] == 'A' || s[i] == '?') g[nxt] += dp[bit];
            if (s[i] == 'B' || s[i] == '?') g[nxt | 1] += dp[bit];
        }
        swap(dp, g);
    }
    rep (i, k, n + 1) {
        ...
        rep (bit, 0, 1 << k) {
            ...
            bool f = 0, f2 = 0;
            rep (j, 0, k / 2) {
                if ((nxt >> j & 1) != (nxt >> (k - 1 - j) & 1)) f = 1;
            }
            if (f && (s[i] == 'A' || s[i] == '?')) g[nxt] += dp[bit];
            nxt |= 1;
            rep (j, 0, k / 2) {
                if ((nxt >> j & 1) != (nxt >> (k - 1 - j) & 1)) f2 = 1;
            }
            if (f2 && (s[i] == 'B' || s[i] == '?')) g[nxt] += dp[bit];
        }
        swap(dp, g);
    }
    mint ans = 0;
    rep (i, 0, 1 << k) ans += dp[i];
    cout << ans.val() << '\n';
}
