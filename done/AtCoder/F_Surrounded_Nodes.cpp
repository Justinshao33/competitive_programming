...
void solve() {
    ...
    dfs(dfs, 0, -1);
    vector<mint> itwo(n + 1, 0);
    itwo[n] = mint(2).pow(n).inv();
    for (int i = n - 1; i >= 0; --i) itwo[i] = itwo[i + 1] * 2;
    mint ans = 0;
    rep (i, 0, n) if (adj[i].size() > 1) {
        mint res = 1 - itwo[n - 1];
        for (int v : adj[i]) {
            if (sz[v] < sz[i]) res -= itwo[n - 1 - sz[v]] * (1 - itwo[sz[v]]);
            else res -= itwo[sz[i] - 1] * (1 - itwo[n - sz[i]]);
        }
        res *= itwo[1];
        ans += res;
    }
