...
void solve() {
    ...
    dp[0][0] = 1;
    // rep (i, 1, n + 1) rep (k, 0, 2) {
    //     ll mi = LINF;
    //     for (int j = i; j > 0; --j) {
    //         chmin(mi, a[j]);
    //         dp[i][k] += dp[j - 1][(k - (i - j) % 2 + 2) % 2] * mi;
    //     }
    // }
    // O(n^2) dp ok
    // each element is occupied with the minimum to the right
    // ex: [l1, r1][l2, r2][l3, r3]cur, r1 < r2 < r3, sum[r][k] is dp[l][k]+dp[l+1][k^1]+...+dp[r][k]
    // => dp[cur] = sum[r3] * a[r3] + sum[r2] * a[r2] + sum[r1] * a[r1]
    // parity so annoying, without it, maybe we can maintain a monotone stack
    // k -> k' rule as below:
    // if k   odd: odd->same    even->inverse
    //       even: odd->inverse even->same
    // before put dp[i - 1][k] in each stack:
    // cur = dp[i - 1][k] -> because there is no same value between last dp and current starting point, so k = k'
    //   if a[st[k'].back()] >= a[i]: 
    //      sum[k'] subtract sum_dp * a[st.back()]
    //      cur += sum_dp
    //      pop_back()
    // put {i, cur + dp[i - 1][k]}
    // sum[k] += cur * a[i];
    vector<mint> sum(2, 0);
    vector<vector<pair<int, mint>>> st(2);
    rep (i, 1, n + 1) {
        rep (j, 0, 2) {
            int k = j ^ (i & 1 ^ 1);
            mint cur = dp[i - 1][j];
            while (!st[k].empty() && a[st[k].back().fi] >= a[i]) {
                auto [id, sum_dp] = st[k].back();
                sum[k] -= sum_dp * a[id];
                cur += sum_dp;
                st[k].pop_back();
            }
