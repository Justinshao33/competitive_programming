...
void solve() {
    ...
    vector dp(20, vector<mint>(U + 1, 0));
    dp[0][1] = 1;
    rep (i, 1, 20) {
        rep (j, 1, U + 1) {
            for (int k = 2; j * k <= U; k++) {
                dp[i][j * k] += dp[i - 1][j] * cnt[k];
            }
        }
    }
    vector<mint> ans(U + 1), inv(20);
    rep (i, 1, 20) inv[i] = mint(i).inv();
    rep (i, 2, U + 1) {
        for (int d = 1, x = i; x <= U; d++, x *= i) {
            for (int j = 1; j * d < 20; j++) { // j = L / d
                int L = d * j;
                ans[x] += inv[L] * dp[j][i] * phi[d];
            }
        }
    }
    rep (i, 2, U + 1) cout << ans[i] << " \n" [i == U];
}
