...
void solve() {
    ...
    for (auto &[x, y] : pt) {
        x = lower_bound(all(dx), x) - dx.begin() + 1;
        y = lower_bound(all(dy), y) - dy.begin() + 1;
    }
    sort(all(pt));
    vector cnt(4, vector<int>(n, 0));
    rep (t, 0, 2) {
        FenwickTree<int> bit(ssize(dy));
        rep (i, 0, n) {
            auto &[x, y] = pt[i];
            if (t == 0) {
                cnt[1][i] += i - bit.query(y);
                cnt[2][i] += bit.query(y - 1);
            } else {
                cnt[0][n - 1 - i] += i - bit.query(y);
                cnt[3][n - 1 - i] += bit.query(y - 1);
            }
            bit.mod(y, 1);
        }
        reverse(all(pt));
    ...
    mint ans = 0;
    rep (i, 0, n) {
        mint a = p2[cnt[0][i]] + p2[cnt[2][i]] - 1, b = p2[cnt[1][i]] + p2[cnt[3][i]] - 1;
        ans += p2[n] - a * b;
    }
    cout << ans << '\n';
}
