...
void solve() {
    ...
    int s = n, t = s + 1;
    rep (i, 0, n - 1) {
        if (p[i].fi & 1) G.add_edge(s, i, p[i].se);
        else {
            G.add_edge(i, t, p[i].se);
            if (isp[1 + p[i].fi]) G.add_edge(n - 1, i, LINF);
        }
    }
    rep (i, 0, n - 1) if (p[i].fi & 1) rep (j, 0, n - 1) if (p[j].fi & 1 ^ 1) {
        if (isp[p[i].fi + p[j].fi])
            G.add_edge(i, j, LINF);
    }
    ans += G.flow(s, t);
    G.add_edge(s, n - 1, cnt);
    ll f = G.flow(s, t);
    ans += f;
    ans += (cnt - f) / 2;    
    cout << ans << '\n';
