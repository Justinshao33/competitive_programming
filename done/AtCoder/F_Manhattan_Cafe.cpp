...
void solve() {
    ...
    rep (i, 0, n) cin >> p[i];
    rep (i, 0, n) cin >> q[i];
    vector dp(D + 1, vector<mint>(D + 1, 0));
    dp[0][0] = 1;
    rep (t, 0, n) {
        auto d = dp, ad = dp;
        rep (i, 1, D + 1) rep (j, 0, D + 1) {
            if (j - 1 >= 0) d[i][j] += d[i - 1][j - 1];
            if (j + 1 <= D) ad[i][j] += ad[i - 1][j + 1];
        }
        rep (i, 0, D + 1) fill(all(dp[i]), 0);
        int g = abs(p[t] - q[t]);
        rep (i, 0, D + 1) rep (j, 0, D + 1) {
            // rep (k, 1, D + 1) dp[i][j] += dp[i - k][j - g - k]
            {
                if (i - 1 >= 0 && j - g - 1 >= 0) dp[i][j] += d[i - 1][j - g - 1];
            }
            // rep (k, 1, D + 1) dp[i][j] += dp[i - g - k][j - k]
            {
                if (i - g - 1 >= 0 && j - 1 >= 0) dp[i][j] += d[i - g - 1][j - 1];
            }
            // rep (k, 0, g + 1) dp[i][j] += dp[i - k][j - g + k] (i - g, j) ~ (i, j - g)
            {
                int mn = -min<int>(0, j - g);
                if (i - mn >= 0) dp[i][j] += ad[i - mn][j - g + mn];
                if (i - g - 1 >= 0 && j + 1 <= D) dp[i][j] -= ad[i - g - 1][j + 1];
            }
        }
    }
    mint ans = 0;
    rep (i, 0, D + 1) ans += reduce(all(dp[i]));
    cout << ans << '\n';
}
