...
void solve() {
    int n; cin >> n;
    const int C = 1 << 10;
    const mint bs = mint(LINF - 5);
    auto work = [&]() -> vector<mint> {
        int m; cin >> m;
        ...
        for (int i = n - 1; i >= 0; --i) {
            set<int> mex;
            for (int v : adj[i]) if (v > i) mex.insert(SG[v]);
            while (mex.count(SG[i])) SG[i]++;
        }
        // cout << SG;
        vector<mint> val(C);
        rep (i, 0, n) val[SG[i]] += bs.pow(i + 1);
        return val;
    };
    vector<vector<mint>> SG;
