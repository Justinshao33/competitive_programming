...
void solve() {
    ...
    rep (i, 0, n) rep (bit, 0, 1 << n) if (bit >> i & 1) {
        r[bit] += r[bit ^ (1 << i)];
        b[bit] += b[bit ^ (1 << i)];
    }
    ...
    mint ans = 0;
    rep (bit, 1, 1 << n) {
        if (r[bit] != b[bit]) continue;
        f[bit] = g[bit] = fac[r[bit]];
        /*
        f[S] = \sum \prod g[T] (T is set of partition of S)
        g[S] = f[S] - \sum \prod g[T'] (T' is set of partition of S excluding the partition is the whole set S)
        */
        for (int msk = (bit - 1) & bit; msk > 0; msk = (msk - 1) & bit) {
            if ((bit ^ msk) > msk) break;
            g[bit] -= g[msk] * f[bit ^ msk];
        }
        ans += g[bit] * fac[m - r[bit]];
    }
    ans *= inv[m];
    cout << ans << '\n';
}
