...
void solve() {
    ...
    rep (i, 1, m + 1) {
        ...
        // BR
        dp[i][0][1] += dp[i - 1][0][1];
        if (n > 1) rep (k, 0, 2) dp[i][1][k] += dp[i - 1][1][k];
        // RB
        if (n > 1) rep (k, 0, 2) dp[i][n][k] += dp[i - 1][n][k];
        dp[i][1][1] += dp[i - 1][1][0] + dp[i - 1][1][1];
        // RR
        dp[i][0][1] += dp[i - 1][1][0] + dp[i - 1][1][1];

        rep (j, 2, n + 1) rep (k, 0, 2) { // BB
            dp[i][j][k] += dp[i - 1][j - 1][k];
        }
        rep (j, 2, n) rep (k, 0, 2) { // RB, BR
            dp[i][j][k] += dp[i - 1][j][k] * 2;
        }
        rep (j, 1, n) rep (k, 0, 2) { // RR
            dp[i][j][k] += dp[i - 1][j + 1][k];
        }
    }
    // rep (i, 0, m + 1) rep (j, 0, n + 1) rep (k, 0, 2) {
    //     cout << i << ' ' << j << ' ' << k << ": " << dp[i][j][k].val() << '\n';
    // }
    mint ans = 0;
    rep (j, 0, n + 1) ans += dp[m][j][1];
    cout << ans.val() << '\n';
}
