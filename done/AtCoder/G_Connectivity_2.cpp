...
void solve() {
    int n, m; cin >> n >> m;
    vector<int> edge(1 << n, 0);
    rep (i, 0, m) {
        int a, b; cin >> a >> b;
        a--, b--;
        rep (bit, 0, 1 << n) {
            if ((bit >> a & 1) && (bit >> b & 1)) edge[bit]++;
        }
    }
    // f(S) = connected subgraph
    // g(S) = subgraph
    // g(S) = 2^edge[S]
    // f(S) = g(S) - \sum_{v in T in S and T != S} f(T) * g(S\T)
    vector<mint> f(1 << n, 0), g(1 << n, 0);
    rep (bit, 0, 1 << n) g[bit] = mint(2).pow(edge[bit]);
    rep (mask, 0, 1 << n) {
        f[mask] = g[mask];
        int v = mask & (-mask);
        for (int bit = mask; bit > 0; bit = (bit - 1) & mask) {
            if (bit == mask || ((bit & v) ^ 1)) continue;
            f[mask] -= f[bit] * g[mask ^ bit];
        }
        // cout << mask << ' ' << f[mask].val() << ' ' << g[mask].val() << '\n';
    }
    int all = (1 << n) - 1;
    rep (i, 1, n) {
        mint ans = 0;
        rep (bit, 0, 1 << n) {
            if ((bit & 1) && (bit >> i & 1)) ans += f[bit] * g[all ^ bit];
        }
        cout << ans.val() << '\n';
    }
}
