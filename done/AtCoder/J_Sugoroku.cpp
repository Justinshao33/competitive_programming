...
void solve() {
    ...
    auto dc = [&](auto self, int l, int r) -> void {
        ...
        int mid = l + r >> 1;
        self(self, l, mid);
        auto c = convolution(vector(dp.begin() + l, dp.begin() + mid + 1), vector(A.begin(), A.begin() + (r == n ? n + 1 : r - l + 1)));
        rep (i, mid - l + 1, ssize(c)) {
            if (r != n && l + i > r) break;
            dp[min(n, l + i)] += c[i];
        }
        rep (i, l, r + 1) if (tr[i]) dp[i] = 0;
        self(self, mid + 1, r);
    };
    dc(dc, 0, n);
    cout << dp[n] << '\n';
