...
void solve() {
    int n; cin >> n;
    vector<mint> fac(2 * n + 1), inv(2 * n + 1);
    fac[0] = 1;
    rep (i, 1, 2 * n + 1) fac[i] = fac[i - 1] * i;
    inv[2 * n] = fac[2 * n].inv();
    for (int i = 2 * n; i > 0; --i) inv[i - 1] = inv[i] * i;
    auto H = [&](int N, int K) -> mint {
        if (N <= 0 || K <= 0) return 0;
        return fac[N + K - 1] * inv[K] * inv[N - 1];
    };
    vector<int> c(26);
    rep (i, 0, 26) cin >> c[i];
    ...
    rep (i, 0, 26) {
        for (int j = n; j > 0; --j) {
            rep (k, 1, c[i] + 1) {
                dp[j] += dp[j - k] * H(j - k + 1, k);
            }
        }
        // rep (j, 1, n + 1) cout << dp[j].val() << " \n" [j == n];
    }
    // cout << H(2 - 2 + 1, 2).val() << '\n';
    mint ans = 0;
