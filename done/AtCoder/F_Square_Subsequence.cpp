...
void solve() {
    ...
    int n = ssize(s);
    for (char &c : s) c -= 'a';
    vector to(n + 1, vector<int>(26, n + 1));
    for (int i = n - 1; i >= 0; --i) {
        rep (j, 0, 26) to[i][j] = to[i + 1][j];
        to[i][s[i]] = i + 1;
    }
    vector dp(n + 1, vector (n + 1, vector<mint>(n + 1, 0)));
    // T = [1, i] T = [j, k]
    rep (j, 1, n + 1) dp[0][j][j - 1] = 1;
    rep (i, 1, n + 1) rep (j, 0, n) rep (k, i - 1, n + 1) rep (c, 0, 26) {
        int nxt = to[j][c], nxt2 = to[k][c];
        if (nxt <= i && nxt2 <= n) {
            dp[nxt][i][nxt2] += dp[j][i][k];
        }
    }
    mint ans = 0;
    rep (j, 1, n + 1) rep (k, j, n + 1) ans += dp[j - 1][j][k];
    cout << ans << '\n';
}
