...
void solve() {
    ll L, R; cin >> L >> R;
    auto dfs = [&](auto self, int pos, int more, int less, int lz) -> mint {
        if (pos == -1) return 1;
        if (vis[pos][more][less][lz]) return dp[pos][more][less][lz];
        vis[pos][more][less][lz] = 1;
        mint res = 0;
        int lb = L >> pos & 1, rb = R >> pos & 1;
        // x and y need same msb
        if (lb == 0 || more) // x 0 y 0
            res += self(self, pos - 1, more, rb | less, lz);
        if ((lb == 0 || more) && (rb == 1 || less) && lz) // x 0 y 1
            res += self(self, pos - 1, more, less, lz);
        if (rb == 1 || less) // x 1 y 1
            res += self(self, pos - 1, !lb | more, less, 1);
        return dp[pos][more][less][lz] = res;
    };
    cout << dfs(dfs, 59, 0, 0, 0).val() << '\n';
}
