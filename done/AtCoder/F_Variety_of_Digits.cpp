...
mint cnt[10005][1 << 10][2][2], dp[10005][1 << 10][2][2];

void solve() {
    string s; cin >> s;
    int n = ssize(s);
    int m; cin >> m;
    vector<int> c(m), b(10, -1);
    rep (i, 0, m) cin >> c[i], b[c[i]] = i;
    cnt[0][0][1][1] = 1; // low? / leading zero?
    rep (i, 0, n) {
        rep (f, 0, 2) rep (lz, 0, 2) {
            rep (d, 0, 10) {
                if (f && d > s[i] - '0') continue;
                rep (bit, 0, 1 << m) {
                    int msk = bit;
                    if (b[d] != -1 && !(d == 0 && lz)) msk |= 1 << b[d];
                    cnt[i + 1][msk][f && (d == s[i] - '0')][lz && (d == 0)] += cnt[i][bit][f][lz];
                    dp[i + 1][msk][f && (d == s[i] - '0')][lz && (d == 0)] += 
                        dp[i][bit][f][lz] * 10 + cnt[i][bit][f][lz] * d;
                }
            }
        }
    }
    int msk = (1 << m) - 1;
    cout << dp[n][msk][0][0] + dp[n][msk][1][0] << '\n';
}
