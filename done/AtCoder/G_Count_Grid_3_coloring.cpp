...
void solve() {
    ...
    if (w > h) {
        rep (i, 0, h) rep (j, i + 1, w) swap(g[i][j], g[j][i]);
        swap(h, w);
    }
    vector<int> p3(w + 1);
    p3[0] = 1;
    rep (i, 1, w + 1) p3[i] = p3[i - 1] * 3;    
    vector col(p3[w], vector<int>(w));
    rep (i, 0, p3[w]) {
        int mask = i;
        rep (j, 0, w) {
            col[i][w - 1 - j] = mask % 3;
    ...
    vector<mint> dp(p3[w], 0);
    auto build = [&](auto self, int j, int sum) -> void {
        if (j == w) {
            rep (i, 1, w) if (col[sum][i] == col[sum][i - 1]) return;
            dp[sum] = 1;
            return;
        }
        if (g[0][j] == '?') {
            rep (i, 0, 3) {
                self(self, j + 1, sum * 3 + i);
            }
        } else {
            self(self, j + 1, sum * 3 + g[0][j] - '0');
        }
    };
    build(build, 0, 0);
    rep (i, 1, h) rep (j, 0, w) {
        vector<mint> dp2(p3[w], 0);
        rep (bit, 0, p3[w]) if (dp[bit].val()) {
            bool f = 0;
            rep (k, 1, j) f |= col[bit][k] == col[bit][k - 1];
            rep (k, j + 1, w) f |= col[bit][k] == col[bit][k - 1];
            if (f) continue;
            if (g[i][j] == '?') {
                rep (k, 0, 3) {
                    if (j && k == col[bit][j - 1]) continue;
                    if (k == col[bit][j]) continue;
                    int nxt = bit - col[bit][j] * p3[w - 1 - j] + k * p3[w - 1 - j];
                    dp2[nxt] += dp[bit];
                }
            } else {
                if (j && g[i][j] - '0'== col[bit][j - 1]) continue;
                if (g[i][j] - '0' == col[bit][j]) continue;
                int nxt = bit - col[bit][j] * p3[w - 1 - j] + (g[i][j] - '0') * p3[w - 1 - j];
                dp2[nxt] += dp[bit];
            }
        }
        swap(dp, dp2);
