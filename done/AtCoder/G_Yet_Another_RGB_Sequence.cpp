...
const int MAXN = 3e6 + 5, MOD = 998244353, IINF = 1e9 + 7, MOD2 = 1000000007;
...
void solve() {
    int R, G, B, K; cin >> R >> G >> B >> K; // RG -> K
    mint ans = fac[R + B] * inv[R - K] * inv[K] * inv[B]; // without single Gs
    ans *= H(R + B + 1 - (R - K), G - K); // insert G with H operation (can't insert after R)
    cout << ans.val() << '\n';
}
...
int main() {
    ...
    // cin >> T;
    fac[0] = inv[0] = 1;
    rep (i, 1, 3000000 + 1) fac[i] = fac[i - 1] * i;
    inv[3000000] = fac[3000000].inv();
    for (int i = 3000000; i > 0; --i) inv[i - 1] = inv[i] * i;
    while (T--) {
        solve();
    }
}