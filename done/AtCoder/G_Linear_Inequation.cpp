...
struct Poly {
    ...
    mint operator[](int x) const { return a[x]; }
    auto begin(){ return a.begin(); }
    auto end(){ return a.end(); }
    auto begin() const { return a.cbegin(); }
    auto end() const { return a.cend(); }
    int size() const { return a.size(); }
    Poly& resize(int n){ a.resize(n); return *this; }
    ...
    Poly D(int n = -1) const {
        if(n == -1) n = size();
        if(n == 0) return {};
        Poly ans(size() - 1);
        for(int i = 1; i < size(); i++) ans[i - 1] = a[i] * mint::raw(i);
        return ans;
    }
    Poly integral() const {
        const int n = size();
        Poly ans(n + 1);
        ans[0] = 1;
        for(int i = 0; i < n; i++) ans[i + 1] = ans[i] * mint::raw(i + 1);
        ans[n] = ans[n].inv();
        for(int i = n; i--; ){
            swap(ans[i], ans[i + 1]);
            ans[i + 1] *= ans[i] * a[i];
    ...
    Poly exp(int n = -1) const {
        ...
        for(int m = 2; m < n; m *= 2) {
            ...
            internal::butterfly(x.a);
            for (int i = 0; i < m * 2; i++) x[i] *= z2[i] * M2;
            internal::butterfly_inv(x.a);
            x = x.integral(); x.a.pop_back();
            for(int i = m; i < min(size(), m * 2); i++) x[i] += a[i];
            for(int i = 0; i < m; i++) x[i] = 0;
            for(int i = m; i < m * 2; i++) x[i] *= M2;
            internal::butterfly(x.a);
            for(int i = 0; i < m * 2; ++i) x[i] *= y[i];
            internal::butterfly_inv(x.a);
...
mint BostanMori(Poly P, Poly Q, ll k) {
    while (k > 0) {
        Poly R = Q;
        for (int i = 1; i < R.size(); i += 2) R[i] = -R[i];
        P *= R, Q *= R;
        for (int i = k & 1; i < P.size(); i += 2) P[i / 2] = P[i];
        for (int i = 0; i < Q.size(); i += 2) Q[i / 2] = Q[i];
        P.resize((P.size() + (k & 1 ^ 1)) / 2);
        Q.resize((Q.size() + 1) / 2);
        k >>= 1;
    }
    return P.size() == 0 ? 0 : P[0] / Q[0];
}

void solve() {
    int n; ll m; cin >> n >> m;
    vector<mint> P(1, 1), Q(2, 1);
    Q[1] = -1;
    rep (i, 0, n) {
        int a; cin >> a;
        vector<mint> A(a + 1);
    ...
    cout << BostanMori(Poly(P), Poly(Q), m) << '\n';
}
