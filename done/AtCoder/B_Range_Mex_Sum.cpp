...
void solve() {
    int n, q; cin >> n >> q;
    init(n);
    vector<int> a(n + 1), pmn(n + 2, n), smn(n + 2, n), pre(n + 1);
    rep (i, 1, n + 1) cin >> a[i];
    rep (i, 1, n + 1) {
        if (a[i] != -1) pmn[i] = a[i];
        else pre[i] = 1;
    ...
    vector val(n + 1, vector<mint>(n + 1));
    int aru = pre[n];
    rep (mex, 1, n + 1) {
        int cnt = mex;
        rep (i, 1, n + 1) {
        ...
        rep (i, 0, n + 1) {
            if (aru - cnt >= 0) val[i][mex] = val[i][mex - 1] + Comb(i, cnt) * fac[cnt] * fac[aru - cnt];
        }
    }
    while (q--) {
        int l, r; cin >> l >> r;
        int bdd = min(pmn[l - 1], smn[r + 1]);
        cout << val[pre[r] - pre[l - 1]][bdd] << '\n';
    }
}
