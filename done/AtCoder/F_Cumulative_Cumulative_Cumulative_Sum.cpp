...
void solve() {
    // b_x = \sum a_i
    // c_x = \sum b_i = \sum \sum a_i = \sum (x - i + 1) * a_i
    // d_x = \sum c_i = \sum \sum b_i = \sum (x - i + 1) * b_i
    //     = \sum (x - i + 1) \sum a_i => n(n + 1) / 2
    //     => \sum (x - i + 1)(x - i + 2)/2 * a_i
    // = 1/2 {a_i i^2} - (2x+3)/2 {a_i i} + (x^2+3x+2)/2 {a_i}
    // c = {a_i i^2} b = {a_i i} a = {a_i}
    int n, q; cin >> n >> q;
    FenwickTree<mint> A(n), B(n), C(n);
    ...
    while (q--) {
        ...
        if (t == 1) {
            ...
            cout << (inv * C.query(x) - (2 * x + 3) * inv * B.query(x) + 
                (x * x + 3 * x + 2) * inv * A.query(x)).val() << '\n';
        }
    }
}
