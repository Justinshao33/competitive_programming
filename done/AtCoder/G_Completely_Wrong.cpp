...
void solve() {
    ...
    rep (i, 0, n) {
        vector<mint> po;
        rep (j, 0, min(cc[i], cg[i]) + 1) {
            po.push_back(fac[j] * Comb(cc[i], j) * Comb(cg[i], j));
        }
        p.push_back(po);
        pq.push({ssize(po), i});
    }
    while (ssize(pq) > 1) {
        ...
        auto [s2, j] = pq.top(); pq.pop();
        p[i] = convolution(p[i], p[j]);
        pq.push({s + s2 - 1, i});
    }
    int rt = pq.top().se;
    // cout << p[rt];
