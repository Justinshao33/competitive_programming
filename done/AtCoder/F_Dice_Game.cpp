...
void solve() {
    ...
    auto mul = [&](auto self, int l, int r) -> Poly {
        if (l == r) return Poly({n, -l});
        int mid = l + r >> 1;
        auto L = self(self, l, mid);
        auto R = self(self, mid + 1, r);
        return L * R;
    };
    auto add = [&](auto self, int l, int r, Poly &P) -> pair<Poly, Poly> {
        if (l == r) return { vector<mint>{P[l]}, vector<mint>{1, -l} };
        int mid = l + r >> 1;
        auto [p1, q1] = self(self, l, mid, P);
        auto [p2, q2] = self(self, mid + 1, r, P);
        return {p1 * q2 + q1 * p2, q1 * q2};
    };
    auto comp = [&](Poly cur) -> Poly {
        auto [p0, q0] = add(add, 0, n, cur);
        p0.resize(m + 1), q0.resize(m + 1); 
        auto res = p0 / q0;
        res.resize(m + 1);
        rep (i, 0, m + 1) res[i] *= inv[i];
        return res;
    };
    Poly p(n + 1), q = mul(mul, 0, n - 1);
    p[n] = fac[n];
    // f(e^x) = p(e^x) / q(e^x)
    p = comp(p), q = comp(q);
    Poly f = p / q;
