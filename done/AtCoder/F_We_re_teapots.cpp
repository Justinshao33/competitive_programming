...
void solve() {
    ...
    while (q--) {
        int x, y; cin >> x >> y;
        auto upd = [&](int x) -> void {
            auto it = teapot.lower_bound(x);
            if (it == teapot.end()) return;
            int len = *it - *prev(it), d = a[*it] - a[*prev(it)];
            Mat<mint> tr(2, 2);
            tr.a[0][0] = Comb(len - d, d);
            tr.a[0][1] = Comb(len - d, d - 1);
            tr.a[1][0] = Comb(len - d - 1, d);
            tr.a[1][1] = Comb(len - d - 1, d - 1);
            tree.set(*it - 1, tr);
        };
        if (teapot.count(x)) {
            tree.set(x - 1, e());
        ...
        int len = n - *teapot.rbegin();
        // [1, 0] * M
        cout << M.a[0][0] * f[len] + M.a[0][1] * f[max<int>(0, len - 1)] << '\n';
    }
}
