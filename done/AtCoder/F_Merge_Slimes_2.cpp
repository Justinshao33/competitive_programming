...
info op(info a, info b) {
    return {a.s + b.s, a.s2 + b.s2, a.len + b.len};
}
...
// sum (a + k)^2 = a^2 + k^2 + 2ak

info mapp(func f, info a) {
    return {a.s + f * a.len, a.s2 + mint(f) * f * a.len + mint(2) * a.s * f, a.len};
}
