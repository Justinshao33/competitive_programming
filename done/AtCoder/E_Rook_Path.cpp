...
void solve() {
    ...
    dp[0][0] = 1;
    while (K--) {
        vector g(2, vector<mint>(2, 0));
        g[0][0] = dp[1][0] + dp[0][1];
        g[1][0] = dp[0][0] * (n - 1) + dp[1][1] + dp[1][0] * (n - 2);
        g[0][1] = dp[0][0] * (m - 1) + dp[1][1] + dp[0][1] * (m - 2);
        g[1][1] = dp[1][0] * (m - 1) + dp[0][1] * (n - 1) + dp[1][1] * (n - 2 + m - 2);
        swap(dp, g);
    }
    int x, y, x2, y2; cin >> x >> y >> x2 >> y2;
    if (x == x2 && y == y2) cout << dp[0][0].val() << '\n';
    else if (x == x2) cout << (dp[0][1] / (m - 1)).val() << '\n';
    else if (y == y2) cout << (dp[1][0] / (n - 1)).val() << '\n';
    else cout << (dp[1][1] / (n - 1) / (m - 1)).val() << '\n';
}
