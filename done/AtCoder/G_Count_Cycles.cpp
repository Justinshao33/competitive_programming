...
void solve() {
    ...
    mint ans = 0, inv = mint(2).inv();
    rep (x, 0, n) {
        vector dp(n, vector<mint>(1 << n, 0));
        dp[x][1 << x] = 1;
        rep (bit, 0, 1 << n) if (bit >> x & 1) {
            rep (i, x, n) if (bit >> i & 1) {
                rep (j, x, n) if (edge[i][j] && (bit >> j & 1)) {
                    dp[j][bit] += dp[i][bit ^ (1 << j)] * edge[i][j];
                }
            }
            if (__builtin_popcount(bit) != 2) {
                rep (i, x, n) if ((bit >> i & 1) && edge[i][x]) {
                    debug(x, i, bit, dp[i][bit]);
                    ans += dp[i][bit] * edge[i][x] * inv;
                }
            }
        }
    }
    rep (i, 0, n) rep (j, i + 1, n) {
        ans += mint(1) * edge[i][j] * (edge[i][j] - 1) / 2;
    }
    cout << ans << '\n';
}
