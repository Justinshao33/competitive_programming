...
void solve() {
    string s; cin >> s;
    int n = ssize(s);
    vector dp(1 << 10, vector(3, vector<mint>(2, 0))); // 2^10, sum, <=/<
    dp[0][0][0] = 1;
    rep (i, 0, n) {
        s[i] -= '0';
        vector g(1 << 10, vector(3, vector<mint>(2, 0))); // 2^10, sum, <=/<
        rep (j, 0, 1 << 10) rep (k, 0, 3) rep (f, 0, 2) {
            int b = popcnt(j);
            rep (d, 0, 10) {
                if (!f && d > s[i]) continue;
                int nj = (b + d == 0) ? j : (j | (1 << d)), nk = (k + d) % 3, nf = f | (d < s[i]);
                g[nj][nk][nf] += dp[j][k][f];
            }
        }
        swap(dp, g);
    ...
    mint ans = 0;
    rep (j, 1, 1 << 10) rep (k, 0, 3) rep (f, 0, 2) {
        int a = j >> 3 & 1, b = popcnt(j) == 3, c = k == 0;
        if (a + b + c == 1) {
            ans += dp[j][k][f];
        }
    }
