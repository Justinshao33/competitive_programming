...
void solve() {
    ...
    rep (i, 0, m) {
        vector<mint> g(1 << n, 0);
        rep (bit, 0, 1 << n) if (dp[bit] != 0) {
            vector<int> lcs(n + 1);
            rep (j, 0, n) lcs[j + 1] = lcs[j] + (bit >> j & 1);
            rep (ch, 'a', 'z' + 1) {
                int mask = 0, pre = 0;
                rep (j, 1, n + 1) {
                    int cur = max(pre, lcs[j]);
                    if (ch == s[j - 1]) cur = lcs[j - 1] + 1;
                    if (cur > pre) mask |= 1 << (j - 1);
                    chmax(pre, cur);
                }
                // debug(mask, bit);
                g[mask] += dp[bit];
    ...
    rep (i, 0, 1 << n) ans[__builtin_popcount(i)] += dp[i];
