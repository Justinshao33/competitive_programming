...
void solve() {
    ...
    vector dp(n, vector<vector<mint>> (n + 1, vector<mint>(n + 1)));
    vector<int> sz(n);
    mint ans = 0;
    auto dfs = [&](auto self, int u, int pa) -> void {
        ...
        for (int v : adj[u]) {
            ...
            self(self, v, u);
            dp[v][0][1]++;
            rep (i, 0, sz[u] + sz[v]) rep (j, 0, sz[u] + sz[v]) {
                g[i][j] = 0;
            }
            rep (x, 0, sz[u]) rep (y, 0, sz[u]) {
                rep (a, 0, sz[v] + 1) rep (b, 0, sz[v] + 1) {
                    g[x + a][y + b] += f[x][y] * dp[v][a][b];
                }
            }
            rep (i, 0, sz[u] + sz[v]) rep (j, 0, sz[u] + sz[v]) {
                f[i][j] = g[i][j];
        ...
        rep (i, 0, sz[u]) rep (j, 0, sz[u]) {
            dp[u][i + 1][j] = f[i][j];
        }
        rep (i, k + 1, sz[u] + 1) {
            rep (j, 0, sz[u] + 1) {
                int a = i, b = j + (pa != -1);
                ans += dp[u][i][j] * fac[a] * fac[b] * inv[a + b];
            }
        }
    };
    dfs(dfs, 0, -1);
