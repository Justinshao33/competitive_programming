...
void solve() {
    ...
    vector<int> a(n);
    rep (i, 0, n) cin >> a[i];
    if ((a[n - 1] != -1 && a[n - 1] != n) || (a[n - 1] != n && count(all(a), n) != 0)) {
        cout << 0 << '\n';
        return;
    }
    a[n - 1] = n;
    vector<int> exi(n + 1);
    for (int &x : a) if (x != -1) exi[x] = 1;
    vector<mint> dp(n + 1);
    if (a[0] == -1) {
        rep (i, 1, n + 1) if (!exi[i]) dp[i] = 1;
    } else {
        dp[a[0]] = 1;
    }
    partial_sum(all(exi), exi.begin());
    ...
    rep (i, 1, n) {
        ...
        if (a[i] == -1) {
            rep (j, 1, n + 1) {
                int cnt = j - exi[j] - ng;
                if (cnt > 0) g[j] += cnt * (dp[j] - dp[j - 1]);
                if (exi[j] - exi[j - 1] == 0) g[j] += dp[j - 1] * 2;
            }
        } else {
            rep (j, a[i] + 1, n + 1) g[j] = dp[j] - dp[j - 1];
            g[a[i]] = dp[a[i] - 1] * 2;
        }
        dp = g;
        partial_sum(all(dp), dp.begin());
