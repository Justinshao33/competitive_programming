...
void solve() {
    int n, k; cin >> n >> k;
    vector dp(n + 1, vector (n + 1, vector<mint>(n * n + 1, 0)));
    dp[0][0][0] = 1;
    rep (i, 1, n + 1) {
        rep (j, 0, n + 1) {
            rep (k, 2 * j, n * n + 1) {
                dp[i][j][k] += dp[i - 1][j][k - 2 * j] * (2 * j + 1); // (px, y) or (x, py): 2j, (x, y): 1
                if (j) dp[i][j][k] += dp[i - 1][j - 1][k - 2 * j]; // none
                if (j + 1 <= n) dp[i][j][k] += dp[i - 1][j + 1][k - 2 * j] * (j + 1) * (j + 1); // (px, y) and (x, py): (j + 1) * (j + 1)
            }
        }
    }
    cout << dp[n][0][k] << '\n';
