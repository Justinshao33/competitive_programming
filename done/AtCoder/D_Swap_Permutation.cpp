...
void solve() {
    ...
    mint tot = 0;
    rep (i, 0, n) {
        sum[i] = mint(i) * i - mint(i - 1) * i / 2 + (mint(i + 1) + n - 1) * (n - i - 1) / 2 - mint(i) * (n - i - 1);
        tot += sum[i];
    }
    Mat M(7, 7);
    mint C2 = mint(n - 2) * (n - 3) / 2;
    // {AB, BA, AC, CA, CB, BC, CC}
    M.a = {
        {C2, 1, n - 2, 0, n - 2, 0, 0},
        {1, C2, 0, n - 2, 0, n - 2, 0},
        {1, 0, C2 + n - 3, 1, 0, 1, n - 3},
        {0, 1, 1, C2 + n - 3, 1, 0, n - 3},
        {1, 0, 0, 1, C2 + n - 3, 1, n - 3},
        {0, 1, 1, 0, 1, C2 + n - 3, n - 3},
        {0, 0, 1, 1, 1, 1, C2 + 2 * (n - 4) + 1}
    };
    powpow(M, m - 1);
    mint ans = 0;
    rep (i, 0, n - 1) {
        mint dif = abs(a[i] - a[i + 1]);
        ans += (M.a[0][0] + M.a[0][1]) * dif;
        if (n > 2) ans += (M.a[0][2] + M.a[0][3]) * (sum[a[i]] - dif) / (n - 2);
        if (n > 2) ans += (M.a[0][4] + M.a[0][5]) * (sum[a[i + 1]] - dif) / (n - 2);
        if (n > 3) ans += M.a[0][6] * (tot - 2 * (sum[a[i]] + sum[a[i + 1]] - dif)) / (n - 2) / (n - 3);
    }
    cout << ans.val() << '\n';
}
