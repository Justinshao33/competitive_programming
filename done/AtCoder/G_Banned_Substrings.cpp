...
void solve() {
    ...
    auto check = [&](string &s) -> bool {
        rep (i, 0, m) {
            for (int j = 0; j + ssize(ban[i]) - 1 < ssize(s); j++) {
                if (string(s.begin() + j, s.begin() + j + ssize(ban[i])) == ban[i]) return false;
            }
        }
        return true;
    };
    vector<mint> vec(1 << min<ll>(6, n), 0);
    rep (i, 0, 1 << min<ll>(6, n)) {
        string s = "";
        rep (j, 0, min<ll>(6, n)) {
            if (i >> j & 1) {
                s += 'b';
    ...
    if (n <= 6) {
        cout << accumulate(all(vec), mint(0)) << '\n';
        return;
    }
    n -= 6;
    Mat<mint> mat(1 << 6, 1 << 6);
    rep (i, 0, 1 << 6) {
        string s = "";
        rep (j, 0, 6) {
            if (i >> j & 1) {
