...
pair<vector<int>, vector<mint>> Dijkstra(vector<vector<pii>> &adj, int s) {
    int n = ssize(adj);
    priority_queue<pii, vector<pii>, greater<>> pq;
    vector<int> dis(n, LINF);
    vector<mint> cnt(n, 0);
    dis[s] = 0;
    cnt[s] = 1;
    pq.push({dis[s], s});
...
void solve() {
    ...
    auto [dis, cnt] = Dijkstra(adj, s);
    auto [rdis, rcnt] = Dijkstra(adj, t);
    mint ans = cnt[t] * rcnt[s];
    int mn = dis[t];
    rep (i, 0, n) {
        if (dis[i] + rdis[i] == mn && dis[i] == rdis[i]) {
            ans -= cnt[i] * cnt[i] * rcnt[i] * rcnt[i];
        }
        for (const auto &[v, w] : adj[i]) {
            if (dis[i] + w + rdis[v] == mn) {
                int l = dis[i], r = dis[i] + w, l2 = rdis[v], r2 = rdis[v] + w;
                if (r2 <= l || r <= l2) continue;
                ans -= cnt[i] * cnt[i] * rcnt[v] * rcnt[v];
            }
        }
    }
    cout << ans << '\n';
