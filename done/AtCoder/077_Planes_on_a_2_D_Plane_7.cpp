...
void solve() {
    ...
    Dinic G(n * 2 + 2);
    rep (i, 0, n) G.add_edge(0, i + 1, 1);
    rep (i, 0, n) G.add_edge(i + 1 + n, 2 * n + 1, 1);
    rep (i, 0, n) rep (k, 0, 8) {
        pll cur = {a[i].F + t * dx[k], a[i].S + t * dy[k]};
        if (mp.count(cur)) {
            G.add_edge(i + 1, n + mp[cur] + 1, 1);
        }
    }
    ll P = G.flow(0, 2 * n + 1);
    if (P != n) {
    ...
    cout << "Yes\n";
    rep (i, 1, n + 1) {
        for (auto [j, r, rc] : G.adj[i]) {
            if (j != 0 && rc == 0) {
                rep (k, 0, 8) {
                    if (a[i - 1].F + t * dx[k] == b[j - 1 - n].F && a[i - 1].S + t * dy[k] == b[j - 1 - n].S) {
                        cout << k + 1 << ' ';
                    }
                }
                break;
