...
void solve() {
    ...
    rep (i, 1, n + 1) {
        if (a[i] != -1) {
            rep (j, 0, n) if (j != a[i]) chmax(la[j], i);
        }
        rep (j, 0, n) {
            int L = max(la[j], i - j - 1);
            // if (a[i] != -1 && j != a[i]) continue;
            if (L <= 0) dp[i][j] += 1;
            chmax(L, 1LL);
            if (L <= i - 1) {
                if (j - 1 >= 0) dp[i][j] += pre[i - 1][j - 1] - pre[L - 1][j - 1];
                if (j + 1 < n) dp[i][j] += suf[i - 1][j + 1] - suf[L - 1][j + 1];
            }
            // for (int k = i - 1; k >= 0; --k) {
            //     if (i - k > j + 1) break;
            //     if (k == 0) {
            ...
            //     //     dp[i][j] += dp[k][l];
            //     // }
            //     if (j - 1 >= 0) dp[i][j] += pre[k][j - 1];
            //     if (j + 1 < n) dp[i][j] += suf[k][j + 1];
            //     if (a[k] != -1 && a[k] != j) break;
            // }
        }
        rep (j, 0, n) {
            pre[i][j] = dp[i][j];
