...
void solve() {
    ...
    vector<int> a(n);
    rep (i, 0, n) cin >> a[i];
    vector dp(n, vector<mint>(n, 0)), forest(n, vector<mint>(n, 0));
    rep (i, 0, n) dp[i][i] = forest[i][i] = 1;
    rep (len, 1, n) {
        rep (i, 0, n) {
            int j = len + i;
            if (j >= n) break;
            dp[i][j] += dp[i + 1][j];
            rep (k, i + 2, j + 1) if (a[i + 1] < a[k]) dp[i][j] += dp[i + 1][k - 1] * forest[k][j];
            rep (k, i + 1, j + 1) if (a[i] < a[k]) forest[i][j] += dp[i][k - 1] * forest[k][j];
            forest[i][j] += dp[i][j];
        }
    }
    cout << dp[0][n - 1].val() << '\n';
