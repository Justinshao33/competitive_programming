...
void solve() {
    ...
    rep (bit, 0, 1 << n) rep (i, 0, n) if (bit & (1 << i)) {
        rep (j, i + 1, n) if (bit & (1 << j)) {
            chmax(dp[bit][0], (x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j]));
        }
    }
    rep (bit, 1, 1 << n) {
        rep (i, 1, k) {
            ...
            for (int bit2 = bit; bit2 > 0; bit2 = (bit2 - 1) & bit) {
                dp[bit][i] = min(dp[bit][i], max(dp[bit ^ bit2][i - 1], dp[bit2][0]));
            }
        }
    }
    cout << dp[(1 << n) - 1][k - 1] << '\n';
