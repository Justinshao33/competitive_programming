...
void solve() {
    int n, x; cin >> n >> x;
    init(n);
    vector dp(n + 1, vector (n + 1, vector<mint> (1 << (2 * x - 1), 0)));
    dp[0][0][0] = 1;
    rep (i, 1, n + 1) rep (j, 0, n) {
        rep (k, 0, 1 << (2 * x - 1)) {
            int mask = k >> 1;
            dp[i][j][mask] += dp[i - 1][j][k];
            rep (l, max(1, i - x + 1), min(n + 1, i + x)) {
                int cur = l - (i - x + 1);
                if (mask >> cur & 1 ^ 1) {
                    dp[i][j + 1][mask | (1 << cur)] += dp[i - 1][j][k];
                }
            }
        }
    }
    mint ans = 0;
    rep (j, 0, n + 1) rep (k, 0, 1 << (2 * x - 1)) {
        if (j & 1) ans -= dp[n][j][k] * fac[n - j];
