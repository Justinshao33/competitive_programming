...
void solve() {
    int n, m; cin >> n >> m;
    int q; cin >> q;
    auto sum = [&](mint a0, mint d, mint len) -> mint {
        return len * (2 * a0 + (len - 1) * d) / 2;
    };
    auto calc = [&](int x, int y) -> mint {
        if (x <= 0 || y <= 0) return mint(0);
        // d = 2 * m, -> d2 = 2 * (x + 1) / 2
        int odd = (x + 1) / 2, eve = x - odd;
        mint Odd = sum(sum(1, 2 * m, odd), 2 * odd, (y + 1) / 2);
        mint Eve = sum(sum(2 + m, 2 * m, eve), 2 * eve, y - (y + 1) / 2);
        return Odd + Eve;
    };
    while (q--) {
        int u, d, l, r; cin >> u >> d >> l >> r;
        cout << calc(d, r) - calc(u - 1, r) - calc(d, l - 1) + calc(u - 1, l - 1) << '\n';
    }
}
