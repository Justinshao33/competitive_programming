...
void solve() {
    ...
    auto dfs = [&](auto self, int u, int pa) -> void {
        ...
        for (int v : adj[u]) {
            ...
            rep (i, 0, sz[u] + 1) {
                rep (j, 0, sz[v] + 1) {
                    g[i + j][0] += dp[u][i][0] * (dp[v][j][0] + dp[v][j][2]);
                    g[i + j + 1][1] += dp[u][i][1] * dp[v][j][0];
                    g[i + j][1] += dp[u][i][1] * (dp[v][j][1] + dp[v][j][2]);
                    g[i + j + 1][2] += dp[u][i][0] * dp[v][j][1];
                    g[i + j][2] += dp[u][i][2] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]);
                }
            }
            sz[u] += sz[v];
            rep (i, 0, sz[u] + 1) rep (y, 0, 3) {
                dp[u][i][y] = g[i][y];
                g[i][y] = 0;
            }
        }
    ...
    dfs(dfs, 0, -1);
    rep (i, 0, n + 1) {
        cout << (dp[0][i][0] + dp[0][i][1] + dp[0][i][2]) << '\n';
    }
}
