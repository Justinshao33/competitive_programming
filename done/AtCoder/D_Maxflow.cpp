...
void solve() {
    ...
    rep (i, 0, n) rep (j, 1, m + 1) cin >> g[i][j];
    Dinic G(n * m + 2);
    rep (i, 0, n) rep (j, 1, m + 1) if (g[i][j] == '.') {
        if ((i + j) & 1) {
            G.add_edge(0, i * m + j, 1);
            rep (k, 0, 4) {
                int nx = i + dx[k], ny = j + dy[k];
                if (nx < 0 || ny < 1 || nx >= n || ny > m || g[nx][ny] == '#') continue;
                // cout << '(' << i << ", " << j << ") ";
                // cout << '(' << nx << ", " << ny << ")\n";
    ...
    int ans = G.flow(0, n * m + 1);
    cout << ans << '\n';
    rep (i, 0, n) rep (j, 1, m + 1) if (g[i][j] == '.') {
        if ((i + j) & 1) {
            for (auto [v, r, rc] : G.adj[i * m + j]) {
                if (rc) continue;
                rep (k, 0, 4) {
                    int nx = i + dx[k], ny = j + dy[k];
                    if (nx < 0 || ny < 1 || nx >= n || ny > m || g[nx][ny] == '#') continue;
