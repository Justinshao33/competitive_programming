...
void solve() {
    ...
    rep (i, 0, n) cin >> a[i];
    int h = n / 2;
    vector<vector<int>> S(n + 1), S2(n + 1);
    rep (bit, 0, 1 << h) {
        int res = 0;
        rep (i, 0, h) if (bit >> i & 1) res += a[i];
        S[__builtin_popcount(bit)].push_back(res);
    }
    rep (bit, 0, 1 << (n - h)) {
        int res = 0;
        rep (i, 0, n - h) if (bit >> i & 1) res += a[h + i];
        S2[__builtin_popcount(bit)].push_back(res);
    }
    rep (i, 0, n + 1) {
        sort(all(S[i]));
        sort(all(S2[i]));
    ...
    rep (k, 0, n) {
        mint sum = 0, cnt = 0;
        rep (i, 0, k + 1) {
            int ptr = ssize(S2[k - i]) - 1;
            mint cs = accumulate(all(S2[k - i]), mint(0));
            for (const int &x : S[i]) {
                while (ptr >= 0 && S2[k - i][ptr] + x >= K) {
                    cs -= S2[k - i][ptr--];
                }
                if (ptr < 0) break;
                cnt += ptr + 1;
        ...
        ans += (cnt * tot - sum) * mint(n - k).inv() * Comb(n, k).inv();
    }
    cout << ans << '\n';
}
