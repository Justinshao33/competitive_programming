...
void solve() {
    ...
    init(m + k + 1);
    // (i, 1) -> (n, m)
    // f(n, m) = \sum_0^n i^k C(n - i + m - 1, m - 1) => k + m - 1 + 1 
    // => need (m + k + 1) points
    vector<mint> a(m + k + 1); // [0, m + k + 1)
    rep (i, 0, m + k + 1) a[i] = mint(i).pow(k);
    rep (i, 0, m) partial_sum(all(a), a.begin());
    if (n <= m + k) {
        cout << a[n] << '\n';
        return;
    }
    vector<mint> pre(m + k + 1), suf(m + k + 1);
    rep (i, 0, m + k + 1) pre[i] = (i ? pre[i - 1] : 1) * (n - i);
    for (int i = m + k; i >= 0; --i) suf[i] = (i + 1 <= m + k ? suf[i + 1] : 1) * (n - i);
    mint ans = 0;
    rep (i, 0, m + k + 1) {
        ans += a[i] * ((m + k - i) & 1 ? -1 : 1) * inv[i] * inv[m + k - i] * (i ? pre[i - 1] : 1) * (i == m + k ? 1 : suf[i + 1]);
    }
    cout << ans << '\n';
}
