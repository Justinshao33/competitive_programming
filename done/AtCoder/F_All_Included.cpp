...
struct ACM {
    ...
    mint query(int n, int L) {
        vector dp(idx + 1, vector<mint> (1 << n, 0));
        dp[0][0] = 1;
        rep (l, 0, L) {
            vector g(idx + 1, vector<mint> (1 << n, 0));
            rep (u, 0, idx + 1) {
                rep (bit, 0, 1 << n) {
                    rep (i, 0, 26) {
                        g[tr[u][i]][bit | id[tr[u][i]]] += dp[u][bit];
                    }
                }
            }
            swap(dp, g);
