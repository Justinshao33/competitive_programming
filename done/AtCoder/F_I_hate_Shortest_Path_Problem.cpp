...
info op(info a, info b) {
    return {min(a.mn, b.mn), min(a.val, b.val)};
}

info e() { return {IINF, IINF}; }
...
void solve() {
    ...
    rep (i, 1, n + 1) {
        ...
        if (r != m) {
            info ori = tree.get(r);
            ll mn = min({IINF, ori.mn, r + tree.prod(l, r).val});
            tree.set(r, {mn, mn - r});
        }
        l = tree.max_right(l, [](info x) {
            return x.mn == IINF;
        });
        while (l < r) {
            tree.set(l, {IINF, IINF});
            l = tree.max_right(l, [](info x) {
                return x.mn == IINF;
            });
        }
        int ans = tree.all_prod().mn + i;
        if (ans >= IINF) ans = -1;
        printf("%d\n", ans);
    }
}
