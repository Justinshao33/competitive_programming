...
using mint = atcoder_modint::modint;
mint dp[105][105][105][105];
...
void solve() {
    ...
    dp[1][0][1][1] = 1;
    rep (i, 2, n + 1) {
        rep (j, 0, K + 1) rep (k, 1, n + 1) rep (l, 1, n + 1) {
            dp[i - 1][j][k][l] += dp[i - 1][j][k - 1][l] + dp[i - 1][j][k][l - 1] - dp[i - 1][j][k - 1][l - 1];
        }
        auto calc = [&](int j, int x, int x2, int y, int y2) -> mint {
            if (x > x2 || y > y2) return 0;
            return dp[i - 1][j][x2][y2] - dp[i - 1][j][x - 1][y2] - dp[i - 1][j][x2][y - 1] + dp[i - 1][j][x - 1][y - 1];
        };
        rep (j, 0, K + 1) rep (k, 1, i + 1) rep (l, 1, i + 1) {
            if (j) {
                dp[i][j][k][l] += calc(j - 1, 1, k - 1, 1, l - 1);
                dp[i][j][k][l] += calc(j - 1, k, i, l, i);
            }
            dp[i][j][k][l] += calc(j, 1, k - 1, l, i);
            dp[i][j][k][l] += calc(j, k, i, 1, l - 1);
        }
    }
    mint ans = 0;
    rep (i, 0, n + 1) rep (j, 0, n + 1) ans += dp[n][K][i][j];
    cout << ans << '\n';
}
