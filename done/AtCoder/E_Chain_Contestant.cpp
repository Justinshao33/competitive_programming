...
void solve() {
    ...
    rep (i, 1, n + 1) {
        int c = s[i - 1] - 'A';
        rep (mask, 0, 1 << 10) rep (j, 0, 10) dp[i][mask][j] = dp[i - 1][mask][j];
        rep (mask, 0, 1 << 10) {
            rep (j, 0, 10) {
                if (j == c) dp[i][mask][j] += dp[i - 1][mask][j];
                if (j != c && (mask >> c & 1 ^ 1)) {
                    dp[i][mask | (1 << c)][c] += dp[i - 1][mask][j];
                }
            }
        }
        dp[i][1 << c][c]++;
    ...
    mint ans = 0;
    rep (i, 0, 1 << 10) rep (j, 0, 10) ans += dp[n][i][j];
    cout << ans.val() << '\n';
}
