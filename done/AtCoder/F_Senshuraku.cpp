...
inline mint excatalan(int n, int m, int k) { //(+1) * n, (-1) * m, prefix sum > -k
    if (k > m) return Comb(n + m, m);
    else if (k > m - n) return Comb(n + m, m) - Comb(n + m, m - k);
    return 0;
}
...
void solve() {
    ...
    vector<int> a(n * 2);
    rep (i, 0, n * 2) cin >> a[i];
    vector<mint> ans(n << 1), p2(n << 1), ip2(n << 1);
    p2[0] = 1;
    rep (i, 1, n << 1) p2[i] = p2[i - 1] * 2;
    ip2.back() = p2.back().inv();
    for (int i = n * 2 - 2; i >= 0; --i) ip2[i] = ip2[i + 1] * 2;
    int mx = *max_element(all(a)), mxp = 0, simx = 0;
    rep (i, 0, n) {
        if (a[i << 1] == mx && a[i << 1 | 1] == mx) mxp++;
        else if (a[i << 1] == mx || a[i << 1 | 1] == mx) simx++; 
    }
    // win -> mx + 1
    mint val = 0, val2 = 0;
    rep (i, 0, simx + 1) {
        if (mxp != 0) val += Comb(simx, i) * mint(i + mxp).inv() * ip2[simx + 1];
        if (i != simx) val2 += Comb(simx - 1, i) * mint(1 + i + mxp).inv() * ip2[simx];
    }
    rep (i, 0, n) {
    ...
    if (mxp == 0) {
        int timx = 0, smxp = 0;
        rep (i, 0, n) {
            if (min(a[i << 1], a[i << 1 | 1]) == mx - 1) smxp++;
            else if (max(a[i << 1], a[i << 1 | 1]) == mx - 1) timx++;
        }
        val = val2 = 0;
        rep (i, 0, timx + 1) {
            val += Comb(timx, i) * ip2[simx + timx] * mint(simx + smxp + i).inv();
            if (i != timx) val2 += Comb(timx - 1, i) * ip2[simx + timx] * mint(simx + smxp + i + 1).inv();
        }
        rep (i, 0, n << 1) {
            if (a[i] == mx || (a[i] == mx - 1 && a[i ^ 1] == mx)) {
                ans[i] += val;
            } else if (a[i] == mx - 1 && a[i ^ 1] == mx - 1) {
                ans[i] += val * ip2[1];
            } else if (a[i] == mx - 1) {
                ans[i] += val2;
            }
