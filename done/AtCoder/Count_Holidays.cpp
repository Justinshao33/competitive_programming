...
void solve() {
    ...
    vector<mint> ans(n + 1, 1), suf(n + 2, 1);
    auto calc = [&](int m, int k) -> mint {
        mint res = 0;
        for (int i = 0; i * (k + 2) <= m; i++) {
            if (i & 1) res -= Comb(m - i * (k + 1), i) * p2[m - i * (k + 2)];
            else res += Comb(m - i * (k + 1), i) * p2[m - i * (k + 2)];
        }
        return res;
    };
    rep (i, 0, n) {
        ...
        int len = ptr - i;
        rep (k, 1, len + 1) ans[k] *= calc(len, k) - calc(len - (k + 1), k);
        suf[len + 1] *= p2[len];
        i = ptr;
    }
    rep (i, 1, n + 1) {
