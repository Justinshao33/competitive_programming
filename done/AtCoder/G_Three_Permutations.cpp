...
void solve() {
    ...
    rep (i, 0, 2 * n) if (!vis[i]) {
        ...
        dp2[1][1][0] = 1;
        dp2[1][1][1] = 1;
        rep (j, 2, m) rep (k, 0, m + 1) {
            if (j == m - 1) {
                rep (f, 0, 2) {
                    dp2[j][k][f] = dp2[j - 1][k][f];
                    if (k && !f) dp2[j][k][f] += dp2[j - 2][k - 1][f];
                }
            } else {
                rep (f, 0, 2) {
                    dp2[j][k][f] = dp2[j - 1][k][f];
                    if (k) dp2[j][k][f] += dp2[j - 2][k - 1][f];
                }
            }
        }
        vector<mint> sdp(m + 1), g(n + 1, 0);
        rep (j, 0, m + 1) sdp[j] = dp2[m - 1][j][0] + dp2[m - 1][j][1];
        rep (j, 0, m + 1) {
            rep (k, j, n + 1) {
                g[k] += dp[k - j] * sdp[j];
            }
