...
void solve() {
    int n, m, l; cin >> n >> m >> l;
    init(n);
    auto calc = [&](int L) -> mint {
        vector dp(n + 1, vector<mint>(m + 1));
        dp[0][0] = 1;
        rep (i, 1, n + 1) rep (j, 0, m + 1) {
            // single point
            if (L >= 1) dp[i][j] += dp[i - 1][j];
            // double edge
            if (L >= 2 && i >= 2 && j >= 2) dp[i][j] += dp[i - 2][j - 2] * Comb(n - (i - 2) - 1, 1);
            // add chain
            for (int k = 2; k <= min(i, L) && j >= k - 1; k++) {
                dp[i][j] += dp[i - k][j - (k - 1)] * Comb(n - (i - k) - 1, k - 1) * fac[k] * inv[2];
            }
            // add cycle
            for (int k = 3; k <= min(i, L) && j >= k; k++) {
                dp[i][j] += dp[i - k][j - k] * Comb(n - (i - k) - 1, k - 1) * fac[k - 1] * inv[2];
            }
        }
        return dp[n][m];
    };
    cout << calc(l) - calc(l - 1) << '\n';
}
