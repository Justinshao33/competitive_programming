...
void solve() {
    ...
    rep (i, 0, n) rep (j, 0, n) dp[i][j] = (int)(a[i] > a[j]);
    mint inv = mint(2).inv();
    int qq = q;
    while (q--) {
        int x, y; cin >> x >> y;
        x--, y--;
        dp[x][y] = dp[y][x] = (dp[x][y] + dp[y][x]) * inv;
        rep (i, 0, n) if (i != x && i != y) {
            dp[i][x] = dp[i][y] = (dp[i][x] + dp[i][y]) * inv;
            dp[x][i] = dp[y][i] = (dp[x][i] + dp[y][i]) * inv;
        }
    }
    mint ans = 0;
    rep (i, 0, n) rep (j, i + 1, n) ans += dp[i][j];
    cout << ans * mint(2).pow(qq) << '\n';
}
