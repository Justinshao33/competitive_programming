...
void solve() {
    string s; cin >> s;
    int n = ssize(s);
    // [XXXX, OXXX, OOXX, OOOX] = 1 + 26 + 1 + 1 // 0, 1, 27, 28
    vector<mint> dp(29);
    dp[0] = 1;
    mint inv2 = mint(2).inv(), inv26 = mint(26).inv();
    int q = 0;
    set<int> st;
    for (char c : s) {
        vector<mint> g(29);
        ...
        if (isupper(c)) {
            if (st.count(c)) {
                rep (i, sz, 27) g[27] += dp[i];
            } else {
                rep (i, sz, 27) {
                    mint aru = mint(i - sz) / mint(26 - sz);
                    g[27] += dp[i] * aru;
                    g[i + 1] += dp[i] * (1 - aru);
                }
            }
            ...
            { // upp
                g[1] += dp[0] * inv2;
                rep (i, max<int>(1, sz), 27) {
                    mint aru = i * inv26;
                    g[27] += dp[i] * aru * inv2;
                    g[i + 1] += dp[i] * (1 - aru) * inv2;
                }
                g[27] += dp[27] * inv2;
            }
            { // low
                rep (i, 0, 27) g[i] += dp[i] * inv2;
                g[28] += (dp[27] + dp[28]) * inv2;
            }
        }
        swap(dp, g);
    }
    cout << accumulate(all(dp), mint(0)) * mint(52).pow(q) << '\n';
}
