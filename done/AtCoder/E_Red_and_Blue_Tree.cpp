...
void solve() {
    ...
    vector<int> cnt(n - 1, 0), path;
    auto dfs = [&](auto self, int u, int pa, int target) -> void {
        if (u == target) {
            for (int id : path) cnt[id]++;
            return;
        }
        for (auto [v, i] : adj[u]) {
            if (v == pa) continue;
            path.pb(i);
            self(self, v, u, target);
            path.pop_back();
        }
    };
    rep (i, 0, m - 1) {
    ...
    rep (i, 0, n - 1) {
        rep (j, 0, 200001) {
            ...
            if (j - cnt[i] >= 0) dp[i + 1][j - cnt[i]] += dp[i][j];
            if (j + cnt[i] <= 200000) dp[i + 1][j + cnt[i]] += dp[i][j];
        }
    }
    cout << dp[n - 1][100001 + k].val() << '\n';
}
