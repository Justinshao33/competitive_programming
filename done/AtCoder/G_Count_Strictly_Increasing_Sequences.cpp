...
void solve() {
    ...
    vector<string> s(n);
    rep (i, 0, n) cin >> s[i];
    vector dp(m + 1, vector<vector<mint>>(n + 1, vector<mint>(n + 1, 0)));
    vector vis(m + 1, vector<vector<bool>>(n + 1, vector<bool>(n + 1, 0)));
    auto dfs = [&](auto self, int cur, int l, int r) -> mint {
        if (l == r) return 1;
        if (cur == m) {
            if (r - l == 1) return 1;
            return 0;
        }
        if (vis[cur][l][r]) return dp[cur][l][r];
        vis[cur][l][r] = 1;
        vector<mint> f(r + 1, 0);
        f[l] = 1;
        rep (d, 0, 10) {
            ...
            rep (i, l, r + 1) {
                rep (j, i, r + 1) {
                    g[j] += f[i] * self(self, cur + 1, i, j);
                    if (j == r) break;
                    if (s[j][cur] != '?' && s[j][cur] - '0' != d) break;
                }
            }
            swap(f, g);
        }
        return dp[cur][l][r] = f[r];
    };
    cout << dfs(dfs, 0, 0, n).val() << '\n';
}
