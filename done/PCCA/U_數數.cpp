...
void solve() {
    mint::set_mod(924844033);
    int n, k; cin >> n >> k;
    init(2 * n);
    UnionFind dsu(2 * n);
    rep (i, 0, n) {
        if (i - k >= 0) dsu.Union(i - k + n, i);
        if (i + k < n) dsu.Union(i + k + n, i);
    }
    auto conv = [&](vector<mint> a, vector<mint> b) -> vector<mint> {
        vector<mint> res(ssize(a) + ssize(b) - 1);
        rep (i, 0, ssize(a)) rep (j, 0, ssize(b)) {
            res[i + j] += a[i] * b[j];
        }
        return res;
    };
    vector<mint> dp(1, 1);
    rep (i, 0, 2 * n) if (dsu.find(i) == i) {
        int m = dsu.size(i);
        vector<mint> val(m);
        rep (j, 0, m) val[j] = Comb(m - j, j);
        dp = conv(dp, val); 
    }
    mint ans = 0;
    while (ssize(dp) <= n) dp.push_back(0);
    rep (i, 0, n + 1) {
