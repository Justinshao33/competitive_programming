...
void solve() {
    ...
    rep (i, 0, n) cin >> G[i];
    mint ans = 0;
    string aim = "genshinimpact";
    vector vis(n, vector<bool>(m, 0));
    auto work = [&]() -> void {
        int l = 13, r = 0;
        for (char c : {'i', 'n'}) {
            int a = aim.find(c), b = aim.rfind(c);
            if ((b - a) % 2) continue; 
            chmin(l, a);
            chmax(r, b);
        }
        ...
        int cur = 0;
        while (to[cur] < 13) {
            vector g(n, vector<mint>(m, 0));
            rep (i, 0, n) rep (j, 0, m) if (G[i][j] == aim[cur]) {
                auto dfs = [&](auto self, int x, int y, int now) -> void {
                    vis[x][y] = 1;
                    if (now == to[cur]) {
                        g[x][y] += dp[i][j];
                    } else {
                        rep (k, 0, 4) {
                            ...
                            if (nx < 0 || ny < 0 || nx >= n || ny >= m || vis[nx][ny] || G[nx][ny] != aim[now + 1]) continue;
                            self(self, nx, ny, now + 1);
                        }
                    }
                    vis[x][y] = 0;
                };
    ...
    rep (i, 0, 13) {
        work();
        rotate(aim.begin(), aim.begin() + 1, aim.end());
    }
    cout << ans << '\n';
}
