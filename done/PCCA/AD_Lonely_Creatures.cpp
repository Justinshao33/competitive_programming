...
void solve() {
    ...
    rep (i, 0, n) {
        ...
        int D = b * b - 4 * a * c;
        if (D > 0) {
            seg.push_back({(-b - sqrtl(D)) / (2 * a), (-b + sqrtl(D)) / (2 * a)});
            dic.push_back((-b - sqrtl(D)) / (2 * a));
            dic.push_back((-b + sqrtl(D)) / (2 * a));
        }
    }
    sort(all(dic));
    ...
    vector<pii> seg2;
    auto id = [&](Real x) -> int {
        int p = lower_bound(all(dic), x) - dic.begin();
        return p;
    };
    for (const auto &[a, b] : seg) {
        seg2.push_back({id(a) + 1, id(b) + 1});
    }
    int ans = 0;
    vector<vector<int>> seg3(n + n + 1);
    for (auto [l, r] : seg2) seg3[l].push_back(r);
    FenwickTree<int> bit(5 * n);
    rep (i, 1, n + n + 1) {
        for (int v : seg3[i]) ans += bit.query(i + 1, v - 1);
        for (int v : seg3[i]) bit.mod(v, 1);
    }
    cout << ans << '\n';
}
