...
void solve() {
    ...
    rep (i, 0, dic.size() - 1) {
        seg.insert({dic[i], dic[i + 1]});
    }
    mint ans = 1, inv = mint(k * (k - 1)).inv();
    Mat Base(2, 2);
    Base.a = {{k - 2, k - 1}, {1, 0}};
    // auto M = powpow2(Base, n - 1);
    // cout << M.a[0][1] * k << '\n';
    sort(all(li));
    vector<int> st;
    for (auto [x, t] : li) {
        if (t == 1) {
            ...
            vector<pii> cand;
            auto it = seg.upper_bound({st.back(), n + 1});
            int cnt = 0;
            for (; it != seg.end() && it->fi < x ; it = next(it)) {
                cand.pb(*it);
                cnt += it->se - it->fi;
            }
            auto M = powpow2(Base, cnt + 2 - 1);
            ans *= M.a[0][1] * k * inv;
            for (auto pp : cand) seg.erase(pp);
            st.pop_back();
        }
    }
    int cnt = 0;
    for (auto [l, r] : seg) cnt += r - l;
    auto M = powpow2(Base, cnt - 1);
    ans *= M.a[0][1] * k;
    cout << ans << '\n';
}
