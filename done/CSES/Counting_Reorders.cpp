...
void solve() {
    ...
    // we want # of adjcent char the same -> answer is dp[0] - dp[1] + dp[2] - dp[3]...
    // can obtain that by counting # of interval separating from consecutive same char => dp
    // # of adjcent char the same = n - 1 - (i - 1)
    vector<mint> dp(n + 1); 
    dp[0] = 1;
    int sz = 0;
    rep (c, 0, 26) if (cnt[c]) {
        ...
        rep (j, 0, sz + 1) {
            rep (k, 1, cnt[c] + 1) {
                g[j + k] += dp[j] * Comb(j + k, k) * Comb(cnt[c] - 1, k - 1);
            }
        }
        sz += cnt[c];
        swap(dp, g);
    ...
    mint ans = 0;
    rep (i, 0, n + 1) ans += dp[i] * ((n - i) & 1 ? -1 : 1);
    cout << ans << '\n';
}
