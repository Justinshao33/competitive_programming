...
void solve() {
    int n; cin >> n;
    vector<mint> dp(1 << 18, 0);
    int mask = (1 << 18) - 1;
    rep (i, 0, n) {
        int x; cin >> x;
        dp[x ^ mask]++;
    }
    rep (i, 0, 18) rep (bit, 0, 1 << 18) if (bit >> i & 1) dp[bit] += dp[bit ^ (1 << i)];
    rep (i, 0, 1 << 18) {
        dp[i] = mint(2).pow(dp[i].val());
    }
    rep (i, 0, 18) for (int bit = (1 << 18) - 1; bit >= 0; --bit) if (bit >> i & 1) dp[bit] -= dp[bit ^ (1 << i)];
    rep (i, 0, n + 1) cout << dp[mask ^ i] << ' ';
}
