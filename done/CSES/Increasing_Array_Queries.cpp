...
void solve() {
    ...
    st.pb(n + 1);
    for (int l = n; l > 0; --l) {
        while (a[l] >= a[st.back()]) st.pop_back();
        mod(1, 1, n, l, st.back() - 1, a[l]);
        for (auto [r, i] : que[l]) {
            ans[i] = query(1, 1, n, l, r) - (pre[r] - pre[l - 1]);
        }
        st.pb(l);
    }
