...
void solve() {
    ...
    int non = 0, pem = 0, qem = 0, pem2 = 0, qem2 = 0;
    rep (i, 0, n) {
        bool f = 1;
        rep (j, 0, n) if (p[j] == i || q[j] == i) f = 0;
        non += f;
        if (p[i] == -1) pem2 += 1;
        if (q[i] == -1) qem2 += 1;
        if (p[i] == -1 || q[i] == -1) {
            ...
            f = 1;
            rep (j, 0, n) if (j != i) {
                if (p[j] == num || q[j] == num) f = 0;
            }
            (p[i] == -1 ? pem : qem) += f;
        }
    }
    mint ans = 0;
    rep (i, 0, min(non, emp) + 1) {
        rep (j, 0, pem + 1) {
            rep (k, 0, qem + 1) {
                ans += ((i + j + k) & 1 ? -1 : 1) * Comb(non, i) * Comb(emp, i) * fac[i] * Comb(pem, j) * Comb(qem, k) * fac[pem2 - i - j] * fac[qem2 - i - k];
            }
        }
    }
    cout << ans << '\n';
