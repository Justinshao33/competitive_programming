...
void solve() {
    int n; cin >> n;
    vector dp(n + 1, vector<vector<mint>>(n + 1, vector<mint>(2, 0)));
    // length i, j adjcent not beautiful pairs (not including i), k = i adjcent with i - 1 ?
    dp[1][0][0] = 1;
    rep (i, 1, n) rep (j, 0, i) {
        // i + 1 put beside i
        dp[i + 1][j][1] += dp[i][j][0] * 2;
        // between i - 1 and i => break previous pair
        dp[i + 1][j][1] += dp[i][j][1];
        // not between => cause one more adjecent pair
        dp[i + 1][j + 1][1] += dp[i][j][1];
        rep (k, 0, 2) {
            // i insert between any not good pair => j's possible choices
            // won't be under the same condition as above, because of the j's pairs doesnt include i
            if (j + k - 1 >= 0) dp[i + 1][j + k - 1][0] += dp[i][j][k] * j;
            // else
            dp[i + 1][j + k][0] += dp[i][j][k] * (i - 1 - j);
        }
    }
    cout << dp[n][0][0].val() << '\n';
}
