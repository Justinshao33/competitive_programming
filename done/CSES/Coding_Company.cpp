...
void solve() {
    ...
    vector dp(n / 2 + 2, vector<mint>(accumulate(all(t), 1), 0));
    dp[0][0] = 1;
    int sum = 0;
    rep (i, 0, n) {
        vector g(n / 2 + 2, vector<mint>(accumulate(all(t), 1), 0));
        sum += t[i];
        rep (j, 0, n / 2 + 1) {
            rep (k, 0, sum + 1) {
                if (dp[j][k] == 0) continue;
                g[j][k] += dp[j][k] * (j + 1);     // 加入別人組或自己孤兒組
                g[j + 1][k + t[i]] += dp[j][k];          // 創個新組
                if (j >= 1 && k >= t[i]) { 
                    g[j - 1][k - t[i]] += dp[j][k] * j;  // 加入別人組然後不讓其他人加
                }
            } 
        }
        swap(dp, g);
